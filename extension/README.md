...
One codebase runs on Chrome, Edge, and Firefox. All extension code calls the `browser.*` API namespace through the bundled [`webextension-polyfill`](https://github.com/mozilla/webextension-polyfill) in `vendor/` (Firefox has `browser.*` natively; the polyfill provides the promise-based shim on Chromium). The canonical `manifest.json` declares both `background.service_worker` (for Chrome and Edge) and `background.scripts` (for Firefox), and `extension/build-zips.mjs` produces two store-ready zips per release: one with both background keys (for Firefox AMO, which requires the pairing) and one with `background.scripts` stripped (for Chrome and Edge, since Edge's MV3 validator rejects it). Run `npm run package:extension` to produce them; output lands in `dist/`. The polyfill loader in `background.js` is guarded by a `typeof importScripts === 'function'` check so the same file works in either runtime context (Chrome/Edge load the polyfill via `importScripts` inside the service worker; Firefox loads it as the first entry of the `scripts` array). `browser_specific_settings.gecko` sets the Firefox extension id, a `140.0` minimum (the version that introduced `data_collection_permissions`), and `data_collection_permissions: { required: ["none"] }`.
