...
const BooklistApp = (function() {
  ...
  /**
   * Whether a title is starred for the cover collage and whether the collage
   * is already full. Recomputed on every call: the star button in the
   * preview and the phone edit sheet both read it, and a stale answer would
   * let one of them star past the limit.
   */
  function getStarLimitState(bookItem) {
    ...
    return {
      isStarred: !!bookItem.includeInCollage,
      atLimit: totalCollageCovers >= maxCovers,
      maxCovers,
    };
  }

  /**
   * Star or unstar a title for the cover collage. Shared by the preview's
   * star button and the phone edit sheet so both take one path: the limit
   * check, one undo step, the extras grid refresh, Folio's collage-ready
   * milestone and the auto-generated cover's regeneration.
   * @returns {boolean} whether anything changed
   */
  function toggleBookStar(bookItem) {
    const { isStarred, atLimit } = getStarLimitState(bookItem);
    if (!isStarred && atLimit) return false; // Can't add more if at limit
  ...
  function createListItemControls(bookItem, index, targetPanel) {
    ...
    const { isStarred, atLimit, maxCovers } = getStarLimitState(bookItem);
    ...
    starButton.onclick = () => toggleBookStar(bookItem);
   ...
   */
  // Resolves once the browser has had a chance to paint. The collage is
  // drawn and encoded in one go, and every control that changes it (a
  // layout tile, a dropdown, a color) used to sit visibly stuck for half a
  // second before it could repaint. rAF alone never fires in a background
  ...
  // Encodes a canvas as a JPEG data URL. toBlob encodes asynchronously, so
  // the page stays responsive through the most expensive step of making a
  // 3000x4800 collage (toDataURL blocked for up to ~0.5s); the result is
  // still a data URL because saved states and .booklist files carry the
  // front cover inline. Falls back to toDataURL wherever toBlob fails.
  function encodeCanvasJpeg(canvas, quality) {
    const sync = () => canvas.toDataURL('image/jpeg', quality);
    return new Promise((resolve) => {
      if (typeof canvas.toBlob !== 'function') { resolve(sync()); return; }
      canvas.toBlob((blob) => {
        if (!blob) { resolve(sync()); return; }
        const reader = new FileReader();
  ...
  function generateCoverCollage(opts) {
    ...
    const regenChain = waitForFonts().then(afterNextPaint).then(() => {
      ...
      return encodeCanvasJpeg(canvas, 0.92).then(dataUrl => {
        if (thisGenId !== _collageGenId) return;
        const frontCoverImg = elements.frontCoverUploader.querySelector('img');
  ...
  // Reset modal: the header's Reset button (and its More-menu proxy on
  // phones) opens this. Cancel takes focus so Enter or Space right after
  // opening does nothing destructive.
  function openResetModal() {
    const modal = document.getElementById('reset-modal');
    if (!modal) return;
    modal.style.display = 'flex';
    setTimeout(function() { document.getElementById('reset-cancel-btn')?.focus(); }, 0);
  }
  ...
  function bindResetModalEvents() {
    ...
    document.getElementById('reset-clear-content-btn')?.addEventListener('click', () => {
      closeResetModal();
      clearContentKeepStyles();
    });
    document.getElementById('reset-everything-btn')?.addEventListener('click', () => {
      closeResetModal();
      resetToBlank();
    });
    attachOverlayClickClose(modal, closeResetModal);
    attachEscapeClose(modal, closeResetModal);
  }
  ...
  function openBookEditSheet(bookItem) {
    ...
    resetBookEditDeleteConfirm();
    updateBookEditActions();
    modal.style.display = 'flex';
    // Focus on next tick so the modal is visible first (some browsers
    // refuse focus on display:none → flex transitions in the same tick).
    // Focus the Star button rather than the title field: the sheet only
    // opens at phone width, where a focused text field raises the
    // keyboard over the action buttons before the user has chosen to type.
    // The first ENABLED action, since Star is disabled on a blank slot or
    // when the collage is full.
    const firstAction = modal.querySelector('.book-edit-action:not([disabled]):not([hidden])');
    setTimeout(function() { (firstAction || titleInput).focus(); }, 0);
  }

  // Sync the sheet's action buttons with the book being edited. Called on
  // open and after every action that leaves the sheet open (star, move),
  // since those re-render the list and can change the position, the star
  // state and the collage limit.
  function updateBookEditActions() {
    const book = _bookBeingEdited;
    if (!book) return;
    const index = myBooklist.indexOf(book);
    const isBlank = !!book.isBlank;

    const starBtn = document.getElementById('book-edit-star-btn');
    const starLabel = document.getElementById('book-edit-star-label');
    const starNote = document.getElementById('book-edit-star-note');
    if (starBtn) {
      const { isStarred, atLimit, maxCovers } = getStarLimitState(book);
      const blocked = isBlank || (!isStarred && atLimit);
      starBtn.disabled = blocked;
      starBtn.setAttribute('aria-pressed', String(isStarred));
      if (starLabel) starLabel.textContent = isStarred ? 'Starred for cover' : 'Star for cover';
      if (starNote) {
        starNote.hidden = !(!isBlank && !isStarred && atLimit);
        starNote.textContent = `All ${maxCovers} cover spots are taken. Unstar another title to star this one.`;
      }
    }

    const position = document.getElementById('book-edit-position');
    if (position) position.textContent = index >= 0 ? `${index + 1} of ${MAX_BOOKS}` : '';
    const upBtn = document.getElementById('book-edit-up-btn');
    const downBtn = document.getElementById('book-edit-down-btn');
    if (upBtn) upBtn.disabled = index <= 0;
    if (downBtn) downBtn.disabled = index < 0 || index >= MAX_BOOKS - 1;

    // Same gate as the Magic button beside each title on desktop.
    const draftBtn = document.getElementById('book-edit-draft-btn');
    if (draftBtn) {
      draftBtn.hidden = !(window.LIBRARY_CONFIG && !window.LIBRARY_CONFIG.disableAutodrafter);
      draftBtn.disabled = isBlank || _pendingDescriptions.has(book.key);
    }
    const deleteBtn = document.getElementById('book-edit-delete-btn');
    if (deleteBtn) deleteBtn.disabled = isBlank;
  }

  // Delete asks for a second tap instead of a dialog: on desktop the X is a
  // small target beside the title, but here it is a large button a thumb
  // can hit on the way to something else, and undo sits behind the
  // collapsed zoom bar in Preview.
  let _bookEditDeleteTimer = null;
  function resetBookEditDeleteConfirm() {
    clearTimeout(_bookEditDeleteTimer);
    const btn = document.getElementById('book-edit-delete-btn');
    const label = document.getElementById('book-edit-delete-label');
    if (btn) btn.classList.remove('is-confirming');
  ...
  function bindBookEditSheetEvents() {
    ...
    // Title actions. Star and Move leave the sheet open (and any typed but
    // unsaved text in its fields, which Save still writes); Draft and
    // Delete close it. Each calls the function the desktop control calls.
    const on = (id, fn) => {
      const el = document.getElementById(id);
      if (el) el.addEventListener('click', fn);
    };
    on('book-edit-star-btn', () => {
      if (!_bookBeingEdited) return;
      toggleBookStar(_bookBeingEdited);
      updateBookEditActions();
    });
    const move = (delta) => {
      const book = _bookBeingEdited;
      ...
      const index = myBooklist.indexOf(book);
      if (index < 0) return;
      handleBookReorder(index, index + delta);
      updateBookEditActions();
      // Reaching the top or bottom disables the button just pressed, which
      // would drop keyboard focus out of the sheet; hand it to the other.
      const pressed = document.getElementById(delta < 0 ? 'book-edit-up-btn' : 'book-edit-down-btn');
      const other = document.getElementById(delta < 0 ? 'book-edit-down-btn' : 'book-edit-up-btn');
      if (pressed && pressed.disabled && other && !other.disabled) other.focus();
    };
    on('book-edit-up-btn', () => move(-1));
    on('book-edit-down-btn', () => move(1));
    on('book-edit-draft-btn', () => {
      const book = _bookBeingEdited;
      if (!book) return;
      // Commit typed edits first so the drafter sees the edited title and
      // author, then close: the draft lands in the list, not in this form.
      saveBookEditSheet();
      handleMagicButtonClick(book, { shiftKey: false });
    });
    on('book-edit-delete-btn', () => {
      const book = _bookBeingEdited;
      const btn = document.getElementById('book-edit-delete-btn');
      const label = document.getElementById('book-edit-delete-label');
      if (!book || !btn) return;
      if (!btn.classList.contains('is-confirming')) {
        ...
        clearTimeout(_bookEditDeleteTimer);
        _bookEditDeleteTimer = setTimeout(resetBookEditDeleteConfirm, 4000);
        return;
      }
      resetBookEditDeleteConfirm();
      const index = myBooklist.indexOf(book);
      // Close WITHOUT saving: handleDeleteBook swaps a new blank object into
      // this slot, so a later Save would write to a book no longer listed.
      closeBookEditSheet();
      if (index >= 0) handleDeleteBook(book, index);
    });
  ...
  /**
   * The gentle reset: blanks every piece of list CONTENT and keeps every
   * style, so a finished list's look can be reused for the next one.
   * Unlike resetToBlank() this is an ordinary undoable edit, built the
   * way applyLook() is: patch a copy of serializeState() and apply it.
   *
   * Content (cleared): books, extra collage covers, the front cover image
   * (generated or uploaded), cover header text, list name, QR URL, QR
   * blurb and custom QR image. Kept: branding, styles, layout, collage
   * count and the show/hide toggles. The QR fields are content by the
   * owner's call: they usually change to fit each list, while the
   * branding logo is the library's and stays.
   *
   * If serializeState() gains a new content field, decide here whether
   * this clears it.
   */
  function clearContentKeepStyles() {
    if (_tourActive) return;
    pushUndo('clear-content');

    const st = serializeState();
    st.meta.listName = '';
    st.books = []; // applyState pads the list with blank slots
    st.extraCollageCovers = [];
    st.ui.coverTitle = '';
    st.ui.coverLineTexts = ['', '', ''];
    st.ui.qrCodeUrl = '';
    st.ui.qrCodeText = '';
    st.images.frontCover = null;
    st.images.frontCoverIsAutoGenerated = false;
    st.images.customQr = null;
  ...
  const MOBILE_MENU_CONTROLS = [
    ...
    { id: 'reset-blank-button', icon: 'fa-rotate-right', label: () => 'Reset…' },
