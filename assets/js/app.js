...
const BooklistApp = (function() {
  ...
  function _extractImages(state) {
    ...
    if (state.images) {
      ...
      if (state.images.customQr) {
        state.images.customQr = _getImageRef(state.images.customQr);
      }
    }
    return state;
  }
  ...
  function _restoreImages(state) {
    ...
    if (state.images) {
      ...
      if (state.images.customQr && typeof state.images.customQr === 'string' && state.images.customQr.startsWith(_imageRefPrefix)) {
        promises.push(_resolveImageRef(state.images.customQr).then(url => { state.images.customQr = url; }));
      }
    }
  ...
  function cacheElements() {
    elements = {
      ...
      showcaseSettings: document.getElementById('showcase-settings'),
      ...
      qrCodeUploader: document.getElementById('qr-code-uploader'),
      qrCodeCustomImg: document.getElementById('qr-code-custom-img'),
  ...
  function updateTiltedSettingsVisibility() {
    ...
    if (elements.showcaseSettings) {
      elements.showcaseSettings.style.display = selectedLayout === 'showcase' ? 'block' : 'none';
    }
  ...
  function createListItemControls(bookItem, index, targetPanel) {
    ...
    // Showcase toggle (render this cover at double size in the Showcase
    // collage layout). Only rendered when the Showcase layout is selected
    // — on the other layouts the flag has no visible effect, so showing
    // the control would just be confusing — and only on starred books,
    // since showcasing implies collage membership. The flag itself
    // persists across layout switches, so toggling layouts doesn't lose
    // the user's picks.
    let showcaseButton = null;
    const collageLayoutNow = elements.collageLayoutSelector?.querySelector('.layout-option.selected')?.dataset.layout || 'classic';
    if (collageLayoutNow === 'showcase' && isStarred) {
      const isShowcased = !!bookItem.featureInCollage;
      showcaseButton = document.createElement('button');
      showcaseButton.className = 'showcase-button';
      if (isShowcased) showcaseButton.classList.add('active');
      showcaseButton.innerHTML = '<i class="fa-solid fa-up-right-and-down-left-from-center"></i>';
      showcaseButton.title = isShowcased
        ? 'Stop showcasing this title'
        : 'Showcase this title (double size in the collage)';
      showcaseButton.setAttribute('aria-label', 'Toggle showcase size in cover collage');
      showcaseButton.setAttribute('aria-pressed', isShowcased ? 'true' : 'false');
      showcaseButton.onclick = () => {
        pushUndo('toggle-showcase');
        bookItem.featureInCollage = !bookItem.featureInCollage;
        debouncedSave();
        renderBooklist();
        if (window.folio) window.folio.react('nod');
        // Auto-regenerate so the size change is visible immediately,
        // same trigger conditions as the star toggle.
        const frontCoverImg = elements.frontCoverUploader?.querySelector('img');
        if (frontCoverImg?.dataset.isAutoGenerated === 'true') {
          if (BookUtils.hasEnoughCoversForCollage(myBooklist, extraCollageCovers, getCollageCoverCount())) {
            generateCoverCollage();
    ...
    if (showcaseButton) controlsDiv.appendChild(showcaseButton);
  ...
  function generateCoverCollage() {
    ...
    // Parallel showcase flags, index-aligned with allCovers. Book covers
    // carry the per-book showcase toggle; extra collage covers (13-20)
    // can't be showcased. Consumed only by the Showcase layout.
    const allCoverFeatured = [
      ...booksWithCovers.map(b => !!b.featureInCollage),
      ...extraCoverUrls.map(() => false)
    ].slice(0, maxCovers);
    ...
    waitForFonts().then(() => {
      ...
      // Build images and their showcase flags together so the two stay
      // index-aligned even when some loads were rejected and filtered out.
      const images = [];
      const featuredFlags = [];
      results.forEach((r, i) => {
        if (r.status === 'fulfilled') {
          images.push(r.value);
          featuredFlags.push(allCoverFeatured[i]);
        }
      });
      if (images.length === 0) {
      ...
      layoutOptions.featuredFlags = featuredFlags;
      ...
      switch (selectedLayout) {
        ...
        case 'tilted':
          ...
          break;
        case 'showcase':
          drawLayoutShowcase(ctx, canvas, images, styles, shouldStretchCovers, layoutOptions);
          break;
        case 'classic':
        default:
  ...
  /**
   * Layout: Showcase
   * Mixed-size quilt: most covers render at 1×1 unit, a few showcased
   * covers render at exactly 2×2 units. The unit cell keeps the 0.75
   * book aspect, so a 2×2 cover is geometrically identical to four
   * singles plus their gutters — the grid tiles perfectly with no
   * cropping and no holes. This is the only layout where covers render
   * at different sizes; which covers go large is user-controlled.
   *
   * Cover assignment: books flagged featureInCollage (the showcase
   * toggle on starred book cards) claim the 2×2 spots in collage order;
   * unclaimed 2×2 spots fall back to the first remaining covers in
   * collage order, so the layout always renders even with nothing
   * flagged. Surplus flagged books simply render at standard size.
   * Extra collage covers (13-20) are never showcased.
   *
   * Hand-tuned per-count patterns (same precedent as Tilted):
   *   12 covers: 5×6 units — 6 doubles + 6 singles  (3 two-row bands)
   *   16 covers: 5×5 units — 3 doubles + 13 singles (row + 2 bands)
   *   20 covers: 5×7 units — 5 doubles + 15 singles (row + 3 bands)
   * The title bar inserts at a segment boundary so it never splits a
   * double. With only 3-4 boundaries per pattern, some title positions
   * coincide: center == lower at 12-count, and center == lower at
   * 16-count. 20-count has enough boundaries for all five positions.
   */
  // Pattern segments: ordered top-to-bottom; `rows` is the segment's
  // unit-row height; `cells` lists covers in reading order as
  // { c: startCol, r: rowWithinSegment, span: 1|2 } (span 2 = a 2×2
  // double anchored at r 0). `titleAfter` maps each title-bar position
  // to how many segments render above the bar.
  const SHOWCASE_PATTERNS = {
    12: {
      cols: 5,
      titleAfter: { top: 0, classic: 1, center: 2, lower: 2, bottom: 3 },
      segments: [
        { rows: 2, cells: [{ c: 0, r: 0, span: 2 }, { c: 2, r: 0, span: 1 }, { c: 3, r: 0, span: 2 }, { c: 2, r: 1, span: 1 }] },
        { rows: 2, cells: [{ c: 0, r: 0, span: 1 }, { c: 1, r: 0, span: 2 }, { c: 3, r: 0, span: 2 }, { c: 0, r: 1, span: 1 }] },
        { rows: 2, cells: [{ c: 0, r: 0, span: 2 }, { c: 2, r: 0, span: 2 }, { c: 4, r: 0, span: 1 }, { c: 4, r: 1, span: 1 }] },
      ],
    },
    16: {
      cols: 5,
      titleAfter: { top: 0, classic: 1, center: 2, lower: 2, bottom: 3 },
      segments: [
        { rows: 1, cells: [{ c: 0, r: 0, span: 1 }, { c: 1, r: 0, span: 1 }, { c: 2, r: 0, span: 1 }, { c: 3, r: 0, span: 1 }, { c: 4, r: 0, span: 1 }] },
        { rows: 2, cells: [{ c: 0, r: 0, span: 2 }, { c: 2, r: 0, span: 1 }, { c: 3, r: 0, span: 2 }, { c: 2, r: 1, span: 1 }] },
        { rows: 2, cells: [{ c: 0, r: 0, span: 1 }, { c: 1, r: 0, span: 2 }, { c: 3, r: 0, span: 1 }, { c: 4, r: 0, span: 1 }, { c: 0, r: 1, span: 1 }, { c: 3, r: 1, span: 1 }, { c: 4, r: 1, span: 1 }] },
      ],
    },
    20: {
      cols: 5,
      titleAfter: { top: 0, classic: 1, center: 2, lower: 3, bottom: 4 },
      segments: [
        { rows: 1, cells: [{ c: 0, r: 0, span: 1 }, { c: 1, r: 0, span: 1 }, { c: 2, r: 0, span: 1 }, { c: 3, r: 0, span: 1 }, { c: 4, r: 0, span: 1 }] },
        { rows: 2, cells: [{ c: 0, r: 0, span: 2 }, { c: 2, r: 0, span: 1 }, { c: 3, r: 0, span: 2 }, { c: 2, r: 1, span: 1 }] },
        { rows: 2, cells: [{ c: 0, r: 0, span: 1 }, { c: 1, r: 0, span: 2 }, { c: 3, r: 0, span: 2 }, { c: 0, r: 1, span: 1 }] },
        { rows: 2, cells: [{ c: 0, r: 0, span: 1 }, { c: 1, r: 0, span: 1 }, { c: 2, r: 0, span: 2 }, { c: 4, r: 0, span: 1 }, { c: 0, r: 1, span: 1 }, { c: 1, r: 1, span: 1 }, { c: 4, r: 1, span: 1 }] },
      ],
    },
  };

  function drawLayoutShowcase(ctx, canvas, images, styles, shouldStretch, options = {}) {
    const canvasWidth = canvas.width;
    const canvasHeight = canvas.height;
    const position = options.titleBarPosition || 'classic';
    let margin = styles.outerMarginPx;
    ...
    const coverCount = images.length;
    const pattern = coverCount <= CONFIG.MIN_COVERS_FOR_COLLAGE
      ? SHOWCASE_PATTERNS[12]
      : (coverCount <= 16 ? SHOWCASE_PATTERNS[16] : SHOWCASE_PATTERNS[20]);
    const numCols = pattern.cols;
    const numRows = pattern.segments.reduce((sum, s) => sum + s.rows, 0);
    const segsAbove = pattern.titleAfter[position] ?? pattern.titleAfter.classic;
    const rowsAbove = pattern.segments.slice(0, segsAbove).reduce((sum, s) => sum + s.rows, 0);
    const rowsBelow = numRows - rowsAbove;

    // Measure the title bar (same draw-clear-redraw trick as Classic)
    const { bgH } = drawTitleBarAt(ctx, styles, canvasWidth, 0);
    ctx.fillStyle = '#FFFFFF';
    ctx.fillRect(0, 0, canvasWidth, bgH + 1);

    // Unit sizing: identical dual-constraint approach to Classic, with
    // the quilt's unit cell standing in for the Classic slot.
    const numVGutters = (position === 'top' || position === 'bottom') ? (numRows - 1) :
                        (rowsAbove > 0 ? rowsAbove - 1 : 0) + (rowsBelow > 0 ? rowsBelow - 1 : 0);
    const marginCount = (position === 'top' || position === 'bottom') ? 1 : 2;
    const totalVerticalSpace = canvasHeight - bgH - marginCount * margin;
    ...
    const unitHeightFromVertical = totalVerticalSpace / (numRows + numVGutters * vGutterRatio);
    const unitWidthFromVertical = unitHeightFromVertical * bookAspect;
    const unitWidthFromHorizontal = canvasWidth / (numCols + (numCols + 1) * hGutterRatio);
    const unitHeightFromHorizontal = unitWidthFromHorizontal / bookAspect;

    let unitWidth, unitHeight;
    if (unitWidthFromVertical <= unitWidthFromHorizontal) {
      unitWidth = unitWidthFromVertical;
      unitHeight = unitHeightFromVertical;
    } else {
      unitWidth = unitWidthFromHorizontal;
      unitHeight = unitHeightFromHorizontal;
    }

    let vGutter = unitHeight * vGutterRatio;
    const hGutter = (canvasWidth - numCols * unitWidth) / (numCols + 1);

    // Balanced leftover redistribution (see drawLayoutClassic). The 5×6
    // 12-count grid matches the canvas aspect almost exactly, so this
    // is mostly a no-op there; the 5×5 and 5×7 grids leave a little.
    if (numVGutters > 0 && marginCount > 0) {
      const availableHeight = canvasHeight - bgH;
      const usedHeight = numRows * unitHeight + numVGutters * vGutter + marginCount * margin;
      const leftover = availableHeight - usedHeight;
      if (leftover > 1) {
        vGutter += (leftover * 0.6) / numVGutters;
        margin += (leftover * 0.4) / marginCount;
      }
    } else if (numVGutters > 0) {
      const usedHeight = numRows * unitHeight + numVGutters * vGutter;
      const leftover = totalVerticalSpace - usedHeight;
      if (leftover > 1) {
        vGutter += leftover / numVGutters;
      }
    ...
    // Cover assignment. Doubles take flagged covers first (collage
    // order), then fall back to the front of the unflagged pool;
    // singles take everything else in collage order.
    const featuredFlags = Array.isArray(options.featuredFlags) ? options.featuredFlags : [];
    const flagged = [];
    const unflagged = [];
    images.forEach((img, i) => {
      (featuredFlags[i] ? flagged : unflagged).push(i);
    });
    const doubleCount = pattern.segments.reduce(
      (sum, s) => sum + s.cells.filter(cell => cell.span === 2).length, 0);
    const doubleQueue = [];
    for (let n = 0; n < doubleCount; n++) {
      if (flagged.length > 0) doubleQueue.push(flagged.shift());
      else if (unflagged.length > 0) doubleQueue.push(unflagged.shift());
    }
    const assigned = new Set(doubleQueue);
    const singleQueue = images.map((_, i) => i).filter(i => !assigned.has(i));

    // Title bar Y, mirroring Classic's flush-top/flush-bottom blocks.
    let titleY;
    if (position === 'top') {
      titleY = 0;
    } else if (position === 'bottom') {
      ...
      const aboveHeight = rowsAbove * unitHeight + (rowsAbove > 0 ? (rowsAbove - 1) * vGutter : 0);
      titleY = aboveHeight + margin;
    }

    const drawSegment = (segment, segY) => {
      for (const cell of segment.cells) {
        const queue = cell.span === 2 ? doubleQueue : singleQueue;
        const imgIdx = queue.shift();
        if (imgIdx === undefined) continue; // defensive: fewer images than slots
        const x = hGutter + cell.c * (unitWidth + hGutter);
        const y = segY + cell.r * (unitHeight + vGutter);
        // A double spans two cells PLUS the gutter between them in both
        // axes — that's what makes it tile exactly against the singles.
        const w = cell.span === 2 ? unitWidth * 2 + hGutter : unitWidth;
        const h = cell.span === 2 ? unitHeight * 2 + vGutter : unitHeight;
        drawCoverImage(ctx, images[imgIdx], x, y, w, h, shouldStretch);
      }
    };

    // Segments above the title bar, flush at the top.
    let currentY = 0;
    for (let s = 0; s < segsAbove; s++) {
      drawSegment(pattern.segments[s], currentY);
      currentY += pattern.segments[s].rows * (unitHeight + vGutter);
    }

    drawTitleBarAt(ctx, styles, canvasWidth, titleY);

    // Segments below, positioned so the last row lands flush at the bottom.
    if (rowsBelow > 0) {
      const belowTotalHeight = rowsBelow * unitHeight + (rowsBelow - 1) * vGutter;
      currentY = canvasHeight - belowTotalHeight;
      for (let s = segsAbove; s < pattern.segments.length; s++) {
        drawSegment(pattern.segments[s], currentY);
        currentY += pattern.segments[s].rows * (unitHeight + vGutter);
      }
    }
  ...
  function drawLayoutTilted(ctx, canvas, images, styles, shouldStretch, options = {}) {
    ...
    //
    // numRows is sized from gridExtent the same way, then rounded UP
    // to a multiple of 12. Without the round-up, a fixed numRows = 12
    // leaves a small triangular gap at the rotated corners when
    // tiltCoverSize is below ~79% at 45° tilt (the slot height shrinks
    // and 12 rows of vStep no longer reach the canvas corners). With
    // the round-up, numRows / 2 is always a multiple of 6 — the LCM
    // of the row cycle lengths used in getImageForCell (3 for 12-count,
    // 4 for 16-count vertical/sequential, 6 for 16-count center and
    // 20-count horizontal). That keeps the row sitting at canvas
    // center on rowMod 0 for every cycle, preserving the
    // "doubled-row groups frame the title bar" alignment the
    // 16/20-count center-horizontal patterns were tuned for.
    //
    // The round-up adds at most 11 rows beyond what the dynamic ceil
    // would give. coverIntersectsBand cheaply rejects any of those
    // that fall off-canvas after rotation, so the cost is just the
    // iteration.
    //
    ...
    if (!masonryMode) {
      ...
      // Both dimensions are now sized from gridExtent in the stretched
      // path, so masonry just reuses numCols / numRows directly.
  ...
  function clearFrontCoverForExtendedMode(count) {
    ...
    if (placeholderText) {
      placeholderText.innerHTML = 'Click to upload a custom cover<br/>(1500 × 2400 px works, 3000 × 4800 px ideal)<br/><br/>OR<br/><br/>Use the Create Cover tool<br/>in the Front Cover tab<br/>(Add covers 13-' + total + ' using the Additional Covers section)';
    }
  ...
  function restoreFrontCoverPlaceholderText() {
    const placeholderText = elements.frontCoverUploader?.querySelector('.placeholder-text');
    if (placeholderText) {
      placeholderText.innerHTML = 'Click to upload a custom cover<br/>(1500 × 2400 px works, 3000 × 4800 px ideal)<br/><br/>OR<br/><br/>Use the Create Cover tool<br/>in the Front Cover tab<br/>(Star 12 titles to include in the collage)';
    }
  }
  ...
  async function exportPdf() {
    ...
    try {
      ...
      showNotification(
        'PDF download started. Print at Default or Actual Size scaling for best quality.',
        'success',
        true,
        5000
      );
  ...
  function serializeState() {
    const books = myBooklist.map(b => ({
      ...
      featureInCollage: !!b.featureInCollage, // showcase toggle (Showcase layout)
    }));
    ...
    // Branding on branded instances is auto-applied from the library
    // config (applyLibraryConfig) on every load, not uploaded by the
    // user. Flag that case so isDraftStateEffectivelyEmpty doesn't treat
    // the library logo as user content and fire a spurious "Draft
    // restored" toast on an otherwise-empty branded instance.
    const brandingSrc = getUploaderImageSrc(elements.brandingUploader);
    const libBrandingPath = (window.LIBRARY_CONFIG && window.LIBRARY_CONFIG.brandingImagePath) || '';
    const brandingIsLibraryDefault = !!(brandingSrc && libBrandingPath
      && brandingSrc.replace(/^\//, '') === String(libBrandingPath).replace(/^\//, ''));
    ...
    return {
      ...
      images: {
        ...
        brandingIsLibraryDefault,
        // customQr is the user-uploaded override of the auto-generated QR.
        // Null when no custom upload — the auto-generated QR (driven by
        // ui.qrCodeUrl) renders instead.
        customQr: (elements.qrCodeCustomImg && !elements.qrCodeCustomImg.hidden && elements.qrCodeCustomImg.src) || null,
      },
    };
  }
  ...
  function applyState(loaded, { silent = false } = {}) {
    ...
    // Custom QR override (if any). _applyCustomQr / _clearCustomQr are
    // populated by setupQrUploaderHandler() at init; safe to call
    // either way.
    const customQr = loaded.images?.customQr || null;
    if (customQr) {
      _applyCustomQr(customQr);
    } else {
      _clearCustomQr();
    }
    ...
    myBooklist = incoming.slice(0, CONFIG.TOTAL_SLOTS).map(b => {
      ...
      return {
        ...
        // Showcase flag only survives on books that kept their star —
        // an unstarred book isn't in the collage at all.
        featureInCollage: shouldStar && !!b.featureInCollage,
      };
    });
  ...
  // QR uploader — slightly different shape than the cover/branding
  // uploaders. The auto-generated QR code stays in #qr-code-canvas; a
  // sibling <img id="qr-code-custom-img"> overlays it when the user
  // uploads a custom image. We toggle visibility between the two
  // rather than replacing src on a single img, so removing the custom
  // image instantly reveals the still-correct generated QR underneath
  // (no regeneration needed; the URL field is preserved).
  //
  // While a custom QR is set, the URL input and Update button are
  // disabled (and the body gets .has-custom-qr for the greyed-out
  // styling) — the URL has no effect on the displayed image until the
  // user removes the custom upload via the delete button.
  function setupQrUploaderHandler() {
    if (!elements.qrCodeUploader) return;
    const fileInput = elements.qrCodeUploader.querySelector('input[type="file"]');
    const deleteBtn = elements.qrCodeUploader.querySelector('.qr-delete-btn');
    const customImg = elements.qrCodeCustomImg;
    ...
    const processImageFile = (file) => {
      ...
      const reader = new FileReader();
      reader.onload = (event) => {
        // 900px matches CONFIG.QR_SIZE_PX exactly — same effective
        // pixel density as the auto-generated QR at 600 DPI export.
        compressImage(event.target.result, { maxDimension: CONFIG.QR_SIZE_PX }).then(compressed => {
          applyCustomQr(compressed);
          debouncedSave();
        });
      };
      reader.onerror = () => showNotification('Failed to read image file.', 'error');
    ...
    if (deleteBtn) {
      deleteBtn.addEventListener('click', (ev) => {
        // Prevent the click from bubbling to the parent <label>, which
        // would otherwise trigger the hidden file input's open dialog.
        ev.preventDefault();
        ev.stopPropagation();
        pushUndo('remove-qr-upload');
        clearCustomQr();
        debouncedSave();
      });
    }
    ...
    // semantics live next to the wiring that uses them. applyCustomQr
    // and clearCustomQr are also called from applyState() below via
    // module-level wrappers so a saved draft restores correctly.
    function applyCustomQr(dataUrl) {
    ...
    // Expose for applyState / reset to call.
    _applyCustomQr = applyCustomQr;
    _clearCustomQr = clearCustomQr;
  }

  // Module-level shims populated by setupQrUploaderHandler so other
  // parts of the app (state restore, reset) can drive the same
  // visibility toggling without re-querying DOM.
  let _applyCustomQr = function() {};
  let _clearCustomQr = function() {};
    ...
    if (elements.collageLayoutSelector) {
      elements.collageLayoutSelector.querySelectorAll('.layout-option').forEach(option => {
        option.addEventListener('click', () => {
          ...
          updateTiltedSettingsVisibility();
          // Re-render the booklist so the per-book showcase toggles
          // appear/disappear with the Showcase layout selection.
          renderBooklist();
          debouncedSave();
          autoRegenerateCoverIfAble();
        });
  ...
  async function enterTourMode() {
    ...
    // Reset QR — clear URL, restore the placeholder image, and drop
