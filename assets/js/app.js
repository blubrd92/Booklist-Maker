...
const BooklistApp = (function() {
  ...
  function applyState(loaded) {
    ...
    // 3. User previously had an auto-generated cover (or no cover at all)
    // This respects user intent - if they uploaded a custom cover, don't overwrite it
    const starredCount = myBooklist.filter(b => !b.isBlank && b.includeInCollage).length;
    const totalCovers = starredCount + extraCollageCovers.length;
    ...
    const wasAutoGenerated = loaded.images?.frontCoverIsAutoGenerated !== false; // true or undefined = auto-generate
