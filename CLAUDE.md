...
- **The drawing: keep the simple style.** A detailed redraw (irises, whiskers, gradient fur, tabby markings) was tried in September 2026 and rejected by the owner as unsettling; what he has now is the ORIGINAL flat drawing with only its proportions changed. The head's contents sit in an inner `<g transform="translate(250 300) scale(0.88) translate(-250 -300)">` inside `#head-group`, so the head is about 12% smaller without redrawing it, and every child animation origin (ears, eyes, nose, mouth, glasses) still works because origins are resolved in that inner group's own coordinates. The ears are taller, the body is a pear (narrow shoulders at y 240, wider base), and the front legs are slimmer and start higher (y 358). Future changes should adjust proportions and poses, not add rendering detail. **Every animation targets only the named groups** (`#head-group`, `.ear-left-anim` / `.ear-right-anim`, `#eyes`, `#nose`, `#mouth`, `#glasses`, `#body`, `#front-legs`, `#tail`) and their transform origins, so shapes inside them can change freely as long as those ids, the origins, and the tail's `fur-orange` + `stripe` classes stay put.
