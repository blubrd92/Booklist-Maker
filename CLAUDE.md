...
- `BookUtils.isCaselessTitle(str)` - Whether a title is SHOUTING rather than merely containing an acronym: true only when there is no lowercase letter anywhere AND there are at least two words. `toTitleCase` inspects one word at a time and preserves any all-uppercase word, which is right for `the USA today` and useless for `EL AMOR EN LOS TIEMPOS DEL CÓLERA`, where every word passes that test and the title returns untouched. This looks at the string as a whole so a caller can flatten the case first. The two-word floor protects a title that genuinely is only an acronym (`SPQR`, `NW`, `IQ`); the accepted cost is that a multi-word all-caps title carrying a real acronym loses it. Strings with no cased letters (numerals, CJK) return false. Used by the Title Capitalization transform's Title Case path.
