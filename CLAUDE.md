...
2. **Quick Add**: `submitQuickAdd()` (in `bindQuickAddEvents()`) is an alternative to Open Library search. The "Quick Add" button below the Search button opens a modal with three inputs (Title, Author, Call Number — title and author required) and a "Apply Title Case" toggle (on by default; turn off for non-English titles like Spanish). Submit is wired through `<form id="quick-add-form">` so pressing Enter in any input submits; the submit button is disabled until both required fields have content. On submit, the title is run through `BookUtils.toTitleCase` if the toggle is on, the author is run through `BookUtils.flipAuthorName` (Last, First → First Last on single-comma names), and the resulting book replaces the next blank slot. Sets the same fields a search-add sets (title, author, callNumber, authorDisplay), so Quick Add and search-add produce indistinguishable books once in the list. Native `<input>` fields strip formatting on paste automatically, so no extra paste handler. Goes through `pushUndo('add-book')` so undo/redo works the same as search-add.
