...
- **Byline words**: `BYLINE_PREFIXES` (By, Por) — the words the Author box's byline buttons write AND the words `getAiDescription`'s author parse strips. One list on purpose; see Key Functional Area 6
...
| `CONFIG.BYLINE_PREFIXES` | `book-utils.js` (`getBylinePrefixWords`/`stripBylinePrefix`/`setBylinePrefix` all read it), `app.js` `initBylinePrefixButtons()` (renders one button per entry) and `getAiDescription`'s author parse (strips the same words), `tests/config.test.js` (shape + no-duplicates suite). Adding a word ships a new button and teaches the drafter parse in the same commit, which is the whole reason it is one list |
