...
- **The drawing: keep the simple style.** Two directions were tried in September 2026 and rejected by the owner: a detailed redraw (irises, whiskers, gradient fur, tabby markings) read as unsettling, and a shrunken head on a pear body read as triangular. The target, from the owner's reference images, is a cute flat cartoon cat with more elegant body proportions: the ORIGINAL full-size head, face and flat colors, sitting on a soft rounded bean of a body (narrow under the chin, round haunches, round bottom; no straight sides), with slim column front legs drawn inside the body outline and small cream feet, and back feet tucked under the haunches. The body is wide at the bottom on purpose: it has to cover the procedural tail's base at (137,416). Future changes should adjust proportions and poses, not add rendering detail. **Every animation targets only the named groups** (`#head-group`, `.ear-left-anim` / `.ear-right-anim`, `#eyes`, `#nose`, `#mouth`, `#glasses`, `#body`, `#front-legs`, `#tail`) and their transform origins, so shapes inside them can change freely as long as those ids, the origins, and the tail's `fur-orange` + `stripe` classes stay put.
