...
- `serializeState()` writes `ui.honeycombBackdrop` (`'soft' | 'sharp' | 'color'`; `applyState` maps the older `'bar'` to `'color'` and falls back to `'soft'` for a missing or unknown value), `ui.honeycombFill` (`{match, color, gradient, color2, direction}`; missing means match the title bar) and `ui.honeycombCoverSizePct` (50 to 100; missing means 100)
