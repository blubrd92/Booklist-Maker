...
- **Micro-reactions**: nod, perk, wince, watch (eye tracking), yawn, startle, satisfied, squish (tactile press-down on single click; nod remains the reaction for app-event triggers like star toggles), flatten (exhausted 2s sag on the click-spam tier — re-triggering mid-animation deliberately does NOT restart it, so sustained spam reads as him staying slumped), plus three idle fidgets (stretch, ear-flick, tail-swish)
