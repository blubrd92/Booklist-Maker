...
- `BookUtils.toSentenceCase(str)` - Lowercases the words, then capitalizes the first *letter* (not the first character, so `¿quien...?` and `«la ciudad...»` capitalize the real first word). For Spanish and other languages that capitalize only the first word and proper nouns in titles. **Acronyms survive**, using the same all-uppercase-token test as `toTitleCase`, so `SPQR: a history of ancient Rome` keeps SPQR and a single-word `SPQR` is left alone. The exception is a title `isCaselessTitle` calls SHOUTING (no lowercase anywhere, 2+ words): there every word looks like an acronym, so protecting them would return the title untouched, which is the one thing this function exists to prevent. Those are flattened whole, and a shouting title carrying a genuine acronym loses it. Two deliberate omissions: no capital after a colon (per RAE, a subtitle naming a partial aspect takes a colon plus lowercase), and no proper-noun detection, so interior proper nouns are flattened and need hand-fixing. Used by the Title Capitalization transform in the Text & Styling tab, NOT by Quick Add.
