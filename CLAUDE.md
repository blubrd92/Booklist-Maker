...
- **The drawing: keep the original.** In September 2026 a series of redesigns was tried and every one was rolled back at the owner's request: a detailed redraw (irises, whiskers, gradient fur) that read as unsettling, a shrunken head on a pear body that read as triangular, a round body wider than the head that read as bottom-heavy, a slim body narrower than the head, a from-scratch three-quarter cartoon cat, and a head-on version of that with various leg shapes. The drawing in index.html is the one from before those attempts. Don't start another redesign unprompted. **Every animation targets only the named groups** (`#head-group`, `.ear-left-anim` / `.ear-right-anim`, `#eyes`, `#nose`, `#mouth`, `#glasses`, `#body`, `#front-legs`, `#tail`) and their transform origins, so if the owner does ask for shape changes, shapes inside them can change freely as long as those ids and origins stay put. (A regex over `d="..."` attributes once also matched `id="tail"` and renamed the group to `t a i l`, silently killing every tail animation; match `\bd="` when rewriting paths, or replace whole blocks.)
