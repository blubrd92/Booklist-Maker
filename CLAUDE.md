...
- **The drawing (redrawn September 2026)**: a sitting tabby with a cat-shaped head (cheek fluff tufts, forehead "M", cheek stripes), curved ears with inner tufts, large gold-green irises with oval pupils, a cat nose with philtrum, whisker pads and fine whiskers, a sitting body with haunches, slim front legs with cream socks, and a tapered chest bib whose point continues over the leg tops (a separate cream shape in `#front-legs`, because the legs otherwise slice the bib with a hard horizontal edge). Fur uses three gradients in the SVG `<defs>` (`#furHead`, `#furBody`, `#furTail`) plus `#iris`; the outline is `#5A3A26` at 3.5. **Every animation targets only the named groups** (`#head-group`, `.ear-left-anim` / `.ear-right-anim`, `#eyes`, `#nose`, `#mouth`, `#glasses`, `#body`, `#front-legs`, `#tail`) and their transform origins, so shapes inside them can be redrawn freely as long as those ids, the origins, the eye centres (190,200)/(310,200) the glasses ring, and the tail's `fur-orange` + `stripe` classes stay put.
