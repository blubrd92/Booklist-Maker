...
- **Tour on phones**: `revealCurrentStep` calls `BooklistApp.showMobileViewFor(target)` so each step shows the view holding its target, and `resolveStepTarget()` in tour.js retargets a step aimed at a hidden header control to the More button, with the step's optional `phoneText` replacing `text` (the Save, Load and closing Folio steps carry one).
