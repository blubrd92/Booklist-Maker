...
assets/
  ...
  js/
    ...
    analytics.js                IIFE. Injects the Cloudflare Web Analytics
                                beacon (cookieless page-view counting).
                                Self-gates to `booklister.org` /
                                `www.booklister.org` only, so it is a no-op
                                on branded library subdomains, `*.pages.dev`
                                previews, and localhost. Loaded from the
                                `<head>` of all 7 user-facing pages (NOT
                                `admin/index.html`). Coupled to the
                                "Analytics" section of `privacy.html` — see
                                the cross-file dependency table.
...
`analytics.js` is also in this layer but is loaded from the `<head>` rather than in the phase-2 sequence above, since it has no dependency on `CONFIG` or anything else. Like `preview-helper.js` it has no public API and self-gates on hostname; on any host other than `booklister.org` / `www.booklister.org` it returns at line 1 with zero DOM and zero network.
...
**Tool layer** (config.js, book-utils.js, app.js, folio.js, tour.js, preview-helper.js, analytics.js): IIFEs exposing globals. No ES6 imports. No build step. This is the original architecture and should be preserved; adding new JS files to this layer adds load-order dependencies in `index.html`.
...
- **Byline words**: `BYLINE_PREFIXES` (By, Por, Par, Di, Ni, Von) — the words the Author box's byline picker writes AND the words `getAiDescription`'s author parse strips. Each entry's `label` names the language for the picker (`Di (Italian)`) and is **display only**; `value` is the bare word both of those paths use. Entries carry an optional `opener: true` permitting a mid-line match; **a word that doubles as a surname particle must not carry it**. One list on purpose; see Key Functional Area 6
...
- `BookUtils.toSentenceCase(str)` - Lowercases the words, then capitalizes the first *letter* (not the first character, so `¿quien...?` and `«la ciudad...»` capitalize the real first word). For Spanish and other languages that capitalize only the first word and proper nouns in titles. **Acronyms survive**, using the same all-uppercase-token test as `toTitleCase`, so `SPQR: a history of ancient Rome` keeps SPQR and a single-word `SPQR` is left alone. The exception is a title `isCaselessTitle` calls SHOUTING (no lowercase anywhere): there every word looks like an acronym, so protecting them would return the title untouched, which is the one thing this function exists to prevent. Those are flattened whole, and a shouting title carrying a genuine acronym loses it. **A word after a token ending in `:`, `?` or `!` IS capitalized.** This deliberately does NOT follow RAE, which wants a Spanish subtitle naming a partial aspect to take a colon plus lowercase: that same rule lowercases every English subtitle too, and English subtitles are the common case here, so the Spanish nicety is left as a one-keystroke manual fix. `?` and `!` end a sentence outright, so the capital is right in both languages. The capital lands on the first *letter*, and a sentence-start token with no letters in it (a standalone `«` or `"`) hands it to the next word rather than swallowing it. Still no proper-noun detection, so interior proper nouns are flattened and need hand-fixing. Used by the Title Capitalization transform in the Text & Styling tab, NOT by Quick Add.
- `BookUtils.transformTextSelection(text, start, end, fn)` - Applies a case transform to the highlighted range of a multi-line string and returns `{text, start, end}` (the new text plus the range to re-highlight). A collapsed range means the whole text, and the returned caret stays collapsed so the next press still means "everything". Works line by line, so Title Case treats each cover line as its own title. For a partial line it runs `fn` on the WHOLE line and keeps only the highlighted characters, so a mid-line `of` stays lowercase under Title Case and a mid-line word is not capitalized by Sentence case; when `fn` changes length (`ß` to `SS`) it transforms the fragment alone instead. A highlighted fragment with no lowercase is lowercased first, because both case functions protect all-caps words as acronyms and would otherwise never undo an UPPERCASE press on part of a line. Used by the cover header's Change capitalization buttons.
- `BookUtils.isCaselessTitle(str)` - Whether a title carries no usable case information at all: no lowercase letter anywhere, but at least one uppercase one. `toTitleCase` inspects one word at a time and preserves any all-uppercase word, which is right for `the USA today` and useless for `EL AMOR EN LOS TIEMPOS DEL CÓLERA`, where every word passes that test and the title returns untouched. This looks at the string as a whole so a caller can flatten the case first. **A one-word all-caps title counts as caseless**, which reverses an earlier two-word floor: that floor protected `SPQR` / `NW` / `IQ` and trapped everything else, because after an UPPERCASE press no button could turn `BELOVED` back. Stuck is worse than wrong-and-fixable, single-word titles far outnumber acronym-only ones on a display list, and `SPQR` is only ever touched when the user presses a button and asks. The acronym protection that matters is untouched: any title containing lowercase is never caseless, so `SPQR: a history of ancient Rome` keeps SPQR. Strings with no cased letters (numerals, CJK) return false. Used by the Title Capitalization transform's Title Case path and by `toSentenceCase`.
- `BookUtils.getBylinePrefixWords(prefixes)` / `BookUtils.stripBylinePrefix(text, prefixes)` / `BookUtils.setBylinePrefix(line, newPrefix, prefixes)` - The byline word machinery behind the Author style box's "Change byline word" buttons. All three read `CONFIG.BYLINE_PREFIXES` unless handed an explicit list (tests only). `stripBylinePrefix` splits a known leading word off an author line and returns `{prefix, rest}`, with `prefix: null` when the line does not open with a word the app knows; the gap after the word is `\s` rather than a literal space because real author lines carry U+00A0 from catalog pastes and can hold line breaks, and the whitespace requirement is what keeps `Byron Smith` from reading as `By` plus `ron`. `getBylineOpenerWords` returns the `opener: true` subset. `setBylinePrefix` rewrites the word: a known word at the very start is replaced, otherwise an OPENER word used as the tail of a hand-written opener (`Edited by`, `Escrito por`) is replaced. Two limits keep that second pass from eating real names: it searches only the first `BYLINE_OPENER_MAX_WORDS` (3) words, so a mid-line `, edited by` cannot cut the author out, and it consults only flagged words, so a surname particle in the list (`Di` and `Von` both ship unflagged) is never read as an opener in `Leonardo di Caprio` or `Ludwig von Beethoven`. A word at position zero is always safe to replace whatever its flag, since a particle cannot start the line. **With no known word near the front the new one is prepended rather than bailing**, which matters because a book typed by hand into the grid keeps `book.author` as the placeholder (the byline field's input handler writes back `authorDisplay` only), so there is no reliable name to anchor on and bailing would mean the buttons did nothing at all on a hand-typed list. The cost is that a genuinely unfamiliar opener gets a word stacked in front of it, visible on the line and undone with one keystroke. Returns `null` when the line is empty or nothing would change. An earlier draft anchored on `book.author` to locate where the name began; it was removed after browser testing showed it was dead for hand-typed books and added nothing the opener scan does not already cover.
...
- `BookUtils.seededRandom(seed)` / `pickRhythmTitle(placed, count, cx, cy, preferred, dir, near)` / `planHoneycomb(opts)` (`opts.scale` is Honeycomb's Cover Size) / `assignHoneycombTitles(cells, count, dx)` / `planJigsaw(aspects, width, availHeight)` / `layoutJigsaw(plan, aspects, width, rowTops)` / `cutJigsawSeams(strips, rand, u)` - The Honeycomb and Jigsaw collage geometry, in canvas pixels with no DOM or canvas access. See Key Functional Area 4 for the two rules they enforce (every title whole once; Staggered-rhythm repeats that never touch). `cutJigsawSeams` defines each shared edge once and hands both pieces the same boundary object, which is why seams always meet; its tab-direction rules (up between cover rows, out of the bar on both edges) are pinned by tests.
...
4. **Cover Collage**: `generateCoverCollage()` renders starred books in 6 user-facing layouts (`data-layout` values in `index.html`'s layout picker, all six in one row of the 340px sidebar in **alphabetical order**): **Classic**, **Honeycomb**, **Jigsaw**, **Masonry**, **Staggered**, **Tilted**. A tile is ~46px wide and "Honeycomb" needs ~57px at a readable size, so the per-tile `.layout-option-label` spans are **visually hidden but kept for screen readers**, and one readable caption under the row (`#collage-layout-name`, `aria-hidden`) names the selected layout, or the hovered tile (muted, `.is-preview`). `initLayoutNameCaption()` keeps it current with a `MutationObserver` on the tiles' classes, because the selection is set from several places (click, `applyState`, reset, and `selectTourLayout` in tour.js) and none of them has to know about the caption. Keep a new layout's tile in alphabetical order and re-measure the row before adding a seventh. The Jigsaw tile icon is **stylized, not to scale**: a to-scale version with the layout's real pieces read as noise at 30px, so it shows a few large pieces with one round tab per seam and the same plain title band as the other tiles. Title bar with 5 position options. **Internally there are 7 draw functions**, because the Classic layout has two visual variants gated on the "Show shelves" toggle: `drawLayoutClassic` (no shelves) and `drawLayoutBookshelf` (Classic geometry with horizontal shelves drawn behind the books). Plus `drawLayoutMasonry`, `drawLayoutStaggered`, `drawLayoutTilted`, `drawLayoutHoneycomb` and `drawLayoutJigsaw`. The dispatch in `generateCoverCollage()` switches on `selectedLayout`, with the `classic` case branching on `showShelvesToggle` to pick between Classic and Bookshelf. So "Bookshelf" is not a separate user-facing layout — it is the Classic layout with shelves on.
   - **Honeycomb and Jigsaw share two rules, and both are tested in `tests/book-utils.test.js`**: (1) every title appears whole at least once, placed before any repeat is dealt (Honeycomb bends this above 100% Cover Size, below); (2) repeats follow Staggered's rhythm (each row reads through the list in order, starting further along than the row above) via `BookUtils.pickRhythmTitle`, which steps along the list past any title that would sit next to itself. All geometry and title dealing lives in `BookUtils` (`planHoneycomb`, `assignHoneycombTitles`, `planJigsaw`, `layoutJigsaw`, `cutJigsawSeams`, `seededRandom`); the app.js draw functions only draw. Both ignore the Stretch toggle (disabled with a hint, as for Masonry). Stroke widths and shadow blurs were tuned on a 750px proof and scale by `canvas.width / 750`; `shadowBlur` is in device pixels and does not follow the transform, so it needs that factor explicitly.
   - **Honeycomb**: pointy-top hexagons running off every edge, cell size chosen as the largest at which every title gets a whole cell clear of the bar (accepting up to 15% smaller cells so a region tall enough for a cell isn't left without covers, unless that leaves a comb of spares). Each cell frames the whole cover, as large as fits without a corner crossing an edge, on a backdrop chosen by **Behind each cover** (`#honeycomb-backdrop` in `#honeycomb-settings`, saved as `ui.honeycombBackdrop`): `soft` (default) is a **softened, lightly darkened wash of that same cover** (`softenedCover`: shrunk to 18px so the upscale blurs it, then a 0.15 darken; a plain `drawImage`, so no canvas `filter` support is needed). It started at 10px and 0.28, and the owner asked for less blur and more of the cover's color; much past ~24px the cover's own title shows through as a ghost in the neighbouring cells. `sharp` is the same cover cropped to fill the cell and darkened slightly, and `color` is a fill: **Match the title bar** (`#honeycomb-color-match`, checked by default) follows the title bar's background, gradient included, and unchecking it reveals the cells' own color, gradient toggle, end color and direction (`#honeycomb-color-custom`), pre-filled from the bar's current background so nothing jumps. A gradient runs within each cell (per-cell keeps tile stamping possible), in the title bar's four directions via `linearGradientForBox`. Saved as `ui.honeycombFill` `{match, color, gradient, color2, direction}`; `color` was once `bar` (title bar color only) and `applyState` still maps `bar` to it. Softened is the default because a sharp copy puts the same picture behind itself at two sizes; the owner wanted the other two available anyway. Neither select carries `.font-select` (see the CSS-classes-that-trigger-JS section), the two color inputs are added to `setupColorPopovers()` by id for the same swatch palette as the title bar's, and the show/hide wrappers are plain divs because a `.control-row`'s `display: flex` would beat `[hidden]`. **Cover Size (%)** (`#honeycomb-cover-size`, saved as `ui.honeycombCoverSizePct`) is Tilted's control exactly: 50 to 110 in steps of 5, clamped on blur, the same pre-edit snapshot pattern (both went from a 100 cap to 110 at the owner's request). **It scales the hexagons, not the cover inside them**: it is passed to `planHoneycomb` as `scale`, which finds the usual largest cell size, multiplies it by the setting and re-lays the comb at that size (preferring a phase with cells on both sides of the bar), so a smaller setting means more, smaller cells and more repeats, while the cover still fills its cell. Shrinking only adds whole cells, so both rules keep holding; tests pin that at 50% and 75%. **Above 100% the whole-title rule bends, knowingly**: 100% is already the largest size at which every title fits whole, so at 110% between 2 and 10 titles (by count and bar position) have no whole cell. `planHoneycomb` then gives every cell a `vis` (the share of the hexagon on the page and clear of the bar's zone), picks the phase whose worst-shown title shows most, and `assignHoneycombTitles` hands the titles without a whole cell to the most visible cut cells, returning `lead` (each title's first placement) beside `whole`. Every title still appears, the worst at 87% to 96% visible in the measured cases; a test pins at least 80% at 12/16/20 titles and every bar position. The alternative, capping the growth at whatever keeps every title whole, would have made 105 and 110 look the same as 100 on most lists. The first version shrank the cover inside a fixed cell, which the owner did not want. The white gaps between cells keep their width at every size. **Each title's cell is drawn once and stamped** wherever that title repeats (`drawHoneycombCell` onto a per-title scratch canvas, copied at whole-pixel positions so it is never resampled): drawing all ~60 cells in place, each with a hexagon clip, backdrop, darken and shadowed cover, rasterized several times slower than any other layout, and every edit regenerates the cover. The title bar floats at Tilted's heights on a white strip and honors the outer margin. **Every lattice cell is kept, including ones mostly under the bar**: `planHoneycomb` used to drop a cell whose middle band fell inside the bar's zone, but that zone is taller than the strip actually drawn (outer margin versus 0.6 of it), so a dropped cell's points showed as white notches along the bar at some positions and Cover Sizes. The strip hides what it covers, and a coverage test (every point outside the zone within r of a cell centre, at every bar position and at 110/100/75/50%) pins it.
   - **Jigsaw**: rows fill the page top to bottom and run off both sides like Staggered; whole pieces sit centered in each row in list order. Covers within 5% of their mean aspect share one piece width in a true grid; otherwise rows are cut free-form, each piece at its cover's own aspect, so nothing is cropped except by the page edge. Seams are straight; the variety is in the tabs (off-center position, size, head, neck, lean), all from `seededRandom(ui.jigsawSeed ...)`, so a saved list keeps its puzzle and **Recut** (`#jigsaw-recut-button` in `#jigsaw-settings`, one `pushUndo('recut-jigsaw')`) bumps the seed. **Tabs between cover rows always point up**, so blanks land at the bottom of a cover near the author and never bite a title; a random-direction option was built in the proofs and deliberately not shipped. **The title bar grows its tabs out of both edges** into the rows beside it, and no tab cuts into it, so it needs no padding beyond its own; it is drawn through `drawTitleBarAt` with `bgSideMarginPx` zeroed, and the outer-margin input is disabled for Jigsaw. The downward tabs land on the top of the covers below the bar, where titles sit. That is **the owner's choice**, made over a straight bottom edge after seeing both: the tabs are outlines, so nothing is hidden, but an outline can cross a cover title there. (Before this the bar was the all-blanks piece, tabbed into from both sides, and carried extra padding so the tabs cleared its text.) **Every cover is drawn whole in its own rectangle, and the tabs are only seams stroked over the covers**, so a tab shows whatever book it sits over and nothing is hidden. The seam is one white stroke (`4 * S`) with a soft shadow (`shadowBlur 3 * S`), which keeps it visible between two light covers; thinner seams were hard to see in the on-screen preview, and a dark line down the middle of the white read as two thin parallel lines on a real list. An earlier version filled each tab from the cover it sprouted from (stretching that cover's edge pixels outward) and it covered real details on the neighboring books on an actual list; don't bring that back.
   - **Rebuilds stay off the critical path.** `generateCoverCollage` waits for `afterNextPaint()` before drawing and encodes through `encodeCanvasJpeg()` (async `toBlob` read back as a data URL, falling back to `toDataURL`), so a dropdown, tile or color change repaints within ~150ms instead of freezing ~0.5-0.75s while the 3000x4800 canvas was drawn and JPEG-encoded in one task. The result is still a data URL on purpose: saved drafts and `.booklist` files carry the front cover inline. The generation id is re-checked after the encode, so a newer rebuild still wins. Separately, `isPlaceholderUrl` in book-utils.js returns early for `data:` URLs: it used to lowercase a full copy of every embedded cover on every call.
5. **PDF Export**: `exportPdf()` pipeline: html2canvas captures at 6.25x scale, jsPDF outputs 11"x8.5" at 600 DPI. Awaits `waitForFonts()` and `waitForImagesDecoded()` before capture to guard against empty branding/cover captures from in-flight image loads. **Blank-capture guard**: each page goes through `capturedPageDataUrl(canvas)`, which throws a `PdfCaptureError` when the canvas has no 2D context, reads fully transparent at five sample points (every real page is drawn on html2canvas's opaque white `backgroundColor`), or encodes to an empty data URL. That is how a device that can't allocate a 600 DPI page canvas (iOS 17 and earlier) fails, silently, and before the guard it saved a blank PDF. Now nothing is saved and the user is told to Save the list and export on a computer. Resolution stays 600 DPI everywhere by the owner's decision; there is no lower-DPI fallback.
6. **Styling**: Per-element font/size/weight/color/line-spacing controls for title, author, description. Cover header text always lives in the single textarea (`#cover-title-input`); the "Style each line separately" toggle (`#cover-advanced-toggle`, id kept for code compat) only swaps the style panel between shared styling and per-line styling, and ships **checked by default** (index.html is the source of truth; `init()` reads the checkbox and the tour's blank reset mirrors it). In per-line mode, the 3 `.line-style-group` blocks map onto the textarea's non-empty lines (trimmed, blanks dropped, via `BookUtils.splitCoverLines`): the first 3 lines get individual styles, lines 4+ inherit Line 3's style (a hint appears in the panel), groups for missing lines are hidden (group 1 always shows), and `updateCoverLineStyleGroups()` keeps group labels quoting each line's text. Serialization: `ui.coverTextModel: 'unified'` marks new-format states; `ui.coverLineTexts` is still written as a DERIVED compat field (first 3 non-empty lines) and is only READ for legacy states (`coverAdvancedMode` true + no `coverTextModel` marker → `applyState` joins the legacy line texts into the textarea). The legacy migration also re-pairs per-line styles with their text via `BookUtils.compactLegacyCoverLineStyles`: the old UI let text sit in line input 2/3 with earlier inputs blank, and joining drops the blanks, so the saved `styles.coverTitle.lines` entries are reordered to follow their text (applied through a shallow-patched copy — `applyState` must never mutate its `loaded` argument, which undo/redo snapshots reuse). Known accepted tradeoffs: (a) a legacy advanced-mode state's never-rendered simple-mode `coverTitle` text is superseded by the migrated line texts; (b) covers with 4+ lines degrade to 3 lines if a `.booklist` file is round-tripped through an OLD deployed version (its schema caps at 3).
   - **Title Capitalization transform** (`#title-case-group`, a `<details>` disclosure nested INSIDE the `[data-style-group="title"]` box in the Text & Styling tab, below the bold/italic/color row, closed by default; summary reads "Change capitalization" and borrows the `.advanced-settings` disclosure look, blue with a rotating triangle, so it reads as something you click rather than a fourth setting label alongside TITLE / AUTHOR / DESCRIPTION). **It is the only non-style control inside any `[data-style-group]` box**, which matters because `captureStyleGroups()` walks those containers with `querySelector` and takes the FIRST match for `.font-select`, `.font-size-input`, `.color-picker`, `.bold-toggle`, `.italic-toggle`, `.line-spacing` and `.align-toggle`; nothing added inside this disclosure may carry those classes, and there is a GOTCHA comment at that function saying so. Earlier iterations had it as a standalone card above the Title box, which read as a fourth setting and put permanent furniture in front of the controls people actually open the panel for. Three buttons, Title Case / Sentence case / UPPERCASE, that rewrite the titles ALREADY in `myBooklist` via `applyTitleCaseTransform(mode)`. This is the after-the-fact fix for a paste that arrived in the wrong convention; Quick Add's own per-tab Title Case checkboxes are untouched and still govern what happens on add. One `pushUndo` entry per press (grouped per mode so Title Case then Sentence case are two steps), skips blank slots and untouched placeholder titles, no-ops with a deliberately non-committal "No titles changed." notification (it must not claim the titles are already in the requested case, because an all-caps title passes through `toTitleCase` untouched), and reports the count it actually changed. Titles only: authors are out of scope because "bell hooks" and "de la Cruz" are correct as catalogued. The cover header has its own twin of these buttons (next bullet). Buttons are disabled when there's nothing to convert, and `updateTitleCaseButtonsState()` also toggles an `is-empty` class on `#title-case-group` that mutes the summary to `var(--text-light)` (overriding its own hover rules), since the disabled buttons are invisible while the disclosure is closed and a fresh empty list would otherwise look identical to one that's ready to convert. The disclosure still opens in that state: the hint inside is worth reading before there are titles. Both are refreshed from `renderBooklist()` and from the inline title field's `oninput`. Deliberately plain buttons rather than a `<select>` plus Apply, partly because a short fixed set doesn't need a picker and partly because a new `.font-select` in that panel would be auto-populated with font names by `populateFontSelects()`. UPPERCASE has no `BookUtils` wrapper (`String.prototype.toUpperCase` is the whole behavior). **The three buttons compose in any order**, which takes a deliberate step: an all-caps title reads to `toTitleCase` as a run of acronyms and would pass through untouched, so the Title Case entry in `TITLE_CASE_TRANSFORMS` routes it through `BookUtils.toSentenceCase` first when `BookUtils.isCaselessTitle` says there is no case information to protect. That composition lives in `app.js`, NOT inside `BookUtils.toTitleCase`: Quick Add's add-time behavior and that function's test suite are deliberately unchanged. An acronym survives inside a mixed-case title (`SPQR: a history of ancient Rome`), but a title that is nothing but capitals is flattened whether it is one word or many, so `BELOVED` converts back and a bare `SPQR` becomes `Spqr`. Prompted by these buttons, the shared focus-ring rule now uses `:focus-visible` rather than `:focus` for every button in the app (see the CSS pitfalls section), so a mouse click no longer leaves any button ringed as though a mode were selected. The no-op notification says "No titles changed." rather than naming a case, so it cannot make a claim the transform has not verified.

   - **Cover header capitalization** (`#cover-case-group`, a `<details>` disclosure in the Front Cover tab's **Cover Header Text** box, between the textarea and Create Cover; NOT in the Cover Header Style box, so the Line style boxes stay pure settings and none of the style-group sweeps reach it). Same three buttons, same `TITLE_CASE_TRANSFORMS` entries, same CSS block as the Title box disclosure, except it has no divider line: an override removes it, so the text box, the disclosure and Create Cover read as one block (the button makes the cover from that text). The line was first moved below the disclosure, then dropped by the owner's choice. The cover header was originally left out as "typed by hand"; it was added because the buttons invite trying looks on a cover, which is a different job from repairing a paste. **Acts on the highlighted part of the textarea** via `BookUtils.transformTextSelection`, or every line when nothing is highlighted, which gives per-line and per-word control with one disclosure instead of one per Line box. `applyCoverCaseTransform(mode)` in app.js: one `pushUndo('convert-cover-case-<mode>')` per press, re-selects the changed range so the next press hits the same words, and does the textarea input handler's work itself (line-group labels, save, debounced regen) since a programmatic write fires no `input`. It also **resets the textarea's pre-edit snapshot** after the write; without that, a snapshot captured on focus before the press is committed by the next keystroke and undoing the typing would also undo the press. The buttons `preventDefault` on `mousedown` so a mouse press never takes focus (or the visible highlight) off the textarea. A textarea keeps its selection after blur but stops drawing it, so `_coverCaseSelectionLive` only honors the selection while focus is in the textarea or has moved from it into the disclosure (the keyboard Tab path); a forgotten highlight elsewhere means "every line". No success notification (the change is visible right there); "No text changed." on a no-op. Buttons disabled and summary muted (`is-empty`) while the header is empty, refreshed from the textarea's input handler and from `toggleCoverMode`, which every programmatic write (applyState, looks, undo/redo, reset, init) passes through.

   - **Byline word transform** (`#byline-prefix-group`, a `<details>` disclosure nested INSIDE the `[data-style-group="author"]` box, summary "Change byline word", closed by default). Same pattern, same CSS block and same `captureStyleGroups()` caveat as the Title Capitalization disclosure above: nothing inside either may carry `.font-select`, `.font-size-input`, `.color-picker`, `.bold-toggle`, `.italic-toggle`, `.line-spacing` or `.align-toggle`. Rewrites the leading word on the author lines already in `myBooklist` via `applyBylinePrefixTransform(prefix)`. **A `<select>` plus an Apply button, not a row of buttons like the Title box**, and the difference is deliberate: capitalization is a closed set of three that will never grow, while this set gains an entry per language the library runs displays in, and six buttons in this panel measure 38px each. Options are **generated by `initBylinePrefixSelect()`** from `CONFIG.BYLINE_PREFIXES` (By, Por, Par, Di, Ni, Von) plus a trailing `No byline word` option carrying the empty value, so adding a language is one `CONFIG` entry and nothing in `index.html` changes. **Option text is the entry's `label`, which names the language** (`Von (German)`), because `Di` and `Ni` are opaque to anyone who did not write the list; the bare `value` is what gets written to the line and what the notification reports. A config test pins that every label leads with its own value. The longest label measures 160px against 179px of usable width in the select, so re-measure before adding one longer than `Por (Spanish, Portuguese)`. `Por` serves Portuguese as well as Spanish. Languages whose byline is a suffix (Chinese's trailing 著) or more than one word (Vietnamese `Tác giả`) cannot be expressed at all, which is a limit of the prefix model rather than an oversight. Apply is wired to the button's click, never the select's `change`: a keyboard user scrolling a focused select passes through every option, and applying per option would run a transform and push an undo entry for each. **The select deliberately omits `.font-select`** even though it wants that look. See the CSS-classes-that-trigger-JS section: that class has two class-driven consumers, and a select inside a `[data-style-group]` box hits both. One `pushUndo('set-byline-prefix-<word>')` per press, skips blank slots and untouched placeholder lines, reports the count it changed plus how many it left alone, and no-ops with "No author lines changed."
     **Deliberately one-shot and nothing is serialized**: books added after a press keep the default "By". That is the right trade for a workflow that pastes a whole list from the extension at once and formats it afterwards, and a render-time setting would mean moving the byline off `authorDisplay`, the hand-editable string the whole feature exists to rewrite.
     **`CONFIG.BYLINE_PREFIXES` is shared with the AI drafter's author parse** in `getAiDescription`, which used to hardcode `/^By\s/i` and now calls `BookUtils.stripBylinePrefix`. That coupling is the point: a word the buttons can write but the parse cannot read means the drafter silently searches for "Por Gabriel Garcia Marquez" as the author name. Adding a word to the list teaches both sides at once, which is why this is a fixed list rather than a free-text field.
     `currentAuthorLine(book)` derives the line the way `renderBooklist` does, because search-added books carry `authorDisplay: null` and have their line built at render time; without it the transform would silently skip every search-added book. Button state is refreshed from `renderBooklist()`, from the inline title field's `oninput` (typing a title un-blanks the slot) AND from the **author field's `oninput`** (typing over the placeholder is what makes a line transformable) and mutes the summary via the same `is-empty` class as the title disclosure.

7. **Looks (cover style presets)** — **currently dark behind `CONFIG.LOOKS_ENABLED: false`**: `#looks-group` ships `hidden` in index.html and `initLooks()` returns without revealing or wiring anything, so the strip and gallery are invisible and unreachable while all code, catalog data, and tests remain live (flip the flag to relaunch; the tour never references Looks, so no tour changes are needed either way). Curated cover-styling bundles in `CONFIG.LOOKS` (schema documented at the constant). Surfaced two ways in the Front Cover tab, placed AFTER the Cover Header Text group (writing the header is the natural first step; looks restyle it): a 3-chip featured strip (`#looks-strip`, order rotates by month via `BookUtils.pickFeaturedLooks` — seasonal looks first, then year-round, then off-season filler) and a gallery modal (`#looks-modal`, opened by `#browse-looks-button`). Applying with an effectively-empty cover text field injects the look's `sampleText` into the textarea as an editable starting point (inside the same undo snapshot), so applying on a fresh tool always produces visible feedback. A look is a **stamp, not a theme**: `applyLook()` patches only the cover slice (styles.coverTitle's colors/gradient/simple/lines plus ui's coverAdvancedMode/collageLayout/showShelves/titleBarPosition, and tilt fields only for tilted looks — margins/padding and book-block text styles are untouched) onto `serializeState()` output and reapplies it through `applyState`, then regenerates the cover if one exists. One `pushUndo('apply-look')` entry; nothing tracks which look is active; no new serialized state. Gallery cards are **real miniature covers**: `renderLookCoverCanvas` runs the actual layout draw functions (`drawLayoutClassic`/`Bookshelf`/`Masonry`/`Staggered`/`Tilted`) at full 3000x4800 on a per-build scratch canvas with 12 cached sample-cover Images (`getLookSampleCoverImages` — real `Image` elements, because the layout functions read `naturalWidth`), then downscales to a portrait thumbnail. Cards fill progressively (one per rAF, cancelled via `_looksBuildSeq` on close/reopen) so the modal opens instantly. Preview text is the user's own cover text when present (rung 2 of the preview ladder) or the look's `sampleText` otherwise (rung 1); `buildLookTitleStyles` is the preview twin of `getCoverTitleStyles` (production-default bar margins) and their output shapes must stay in sync. Disabled during the tour (`_tourActive` guard in applyLook).
...
10. **Branded library auth (gated instances only)**: Login modal, email/password sign-in with visibility toggle, password reset via email. See the Firebase Integration section below for the full flow. The modal also carries the app's only **sign-in-wrap notice** (`.auth-legal-note`, under the Sign in button): "By signing in, you agree to the Terms and Privacy policy." Gated sign-in is the one place in Booklister with an affirmative account action to attach assent to — the public tool deliberately has **no** consent gate or interstitial, since it has no accounts, no payments, and nothing leaves the browser; a footer Terms link on every page is the whole mechanism there, and that's intentional (don't "fix" it by adding a modal). The notice's links are absolute `https://booklister.org/...` URLs (it renders on library subdomains) and open in a new tab so reading them doesn't discard a half-typed sign-in. The admin console carries the same notice at the foot of its sign-in card (`.admin-legal-note`, covering both the Google and email paths) as a deliberately separate implementation, since `admin/` and `assets/` share no code.

## Phone Layout (max-width 768px)

About a tenth of visits are phones. The tool stays desktop-first, but at `isMobileLayout()` widths (the same `max-width: 768px` media query the CSS uses) the chrome is rearranged so the controls and the printed sheet each get the whole screen. **Desktop and the 769-1100px range are pixel-identical to before**; every rule lives in the 768px block, and the phone-only elements are `display: none` by default (`.mobile-more-button, .mobile-more-menu, .mobile-view-switch`).

- **Edit / Preview views.** `#mobile-view-switch`, a bottom bar in the footer's place, flips `body[data-mobile-view]` between `edit` and `preview` via `setMobileView()` in app.js. The sidebar becomes an absolutely positioned overlay that COVERS the preview in Edit view and is `visibility: hidden` in Preview view. **Neither view is ever `display: none`, and that is load-bearing**: `computeFitToWidthZoom` reads `.main-content`'s width, `exportPdf` html2canvas-captures the print pages (a `display: none` or `visibility: hidden` preview captures blank), and the tour's `prepare` hooks scroll inside the sidebar. `applyMobileViewInert()` marks the covered view `inert` (phones only; a `matchMedia` change listener clears it on resize to desktop). Every modal is a body-level sibling of both views, so neither inert nor the visibility toggle reaches them; keep new modals out of `.sidebar` and `.main-content` for the same reason. The zoom/undo bar is hidden in Edit view.
- **Starting view**: Edit (search is where a list begins), switched to Preview after the startup draft restore if the list has titles, unless the user already tapped a view (`_mobileViewChosenByUser`).
- **Title count badge** on the Preview button (`updateMobileViewCount()`, called from `renderBooklist`), so an add made from the Edit view visibly lands somewhere.
- **Header in one row**: logo, Generate PDF, and `#mobile-more-button`. Every other header control is hidden by `.header-actions > :not(#export-pdf-button):not(#mobile-more-button)` and reached through `#mobile-more-menu`, whose items are **proxies that `click()` the real, hidden buttons** (`MOBILE_MENU_CONTROLS` in app.js), so no handler is duplicated. The menu is rebuilt on every open, so labels like Show/Hide Folio track state and branded-only controls (sign-out, the admin link) appear only when present and not `[hidden]`. A new header button needs an entry there to be reachable on phones. The tool page's `.site-footer` is hidden on phones and its links are cloned into the same menu.
- **Content pages on phones** hide the footer too and carry its links in a header menu, `<details class="content-menu">`, whose summary and panel reuse `.mobile-more-button` / `.mobile-more-menu` / `.mobile-more-item` so both headers look alike. No script: the six content pages load none besides analytics, so it opens and closes natively and does not close on a tap elsewhere. **The links are a hand-kept second copy of `.site-footer-nav`** on each page (same order, same `aria-current`). `.content-menu:not([open]) > .mobile-more-menu { display: none }` is load-bearing: the shared menu rule positions the panel `fixed`, and a fixed child escapes `<details>`' own hiding in Chromium, so without it the menu shows while closed.
- **Folio**: never shown on phones; see the Folio section.
- **Tour on phones**: the same six sections and order, adapted where the phone genuinely differs. Three optional step fields, all read only when `isPhoneLayout()` in tour.js is true: `phoneText` (replaces `text`: "tap", no arrow-key tip, the More menu, the Edit/Preview bar), `phoneTarget` (replaces `target`; `null` means a centered panel with no spotlight, used for step 1 and the closing step, whose floating Folio phones never show) and `phoneSheet: true` (the step opens the tap-to-edit sheet on the first sample title via `BooklistApp.openBookEditSheetAt(0)`; every other phone step closes it, as does `exitTour`). The Your Booklist star / reorder / draft-or-description / delete steps use `phoneSheet` and spotlight the sheet's own buttons, since the per-title controls they describe on desktop are hidden on phones. `revealCurrentStep` calls `BooklistApp.showMobileViewFor(target)` so each step shows the view holding its target, and `resolveStepTarget()` also retargets a step aimed at a hidden header control to the More button. **Panel placement**: tour.css makes the panel full width at 768px and `positionPanel()` docks it to the top or bottom edge, whichever half the target is not in, clear of the header and the Edit/Preview bar. Near-full-screen targets (the whole preview, the cover style section) are necessarily partly under the panel. Desktop is untouched: a full desktop run never opens the sheet and keeps the 340px panel.
- **A tap on a title always opens the sheet, because the fields cannot take focus on phones.** `createListItemDetails` creates the title / author / description fields with `contentEditable` false at phone width, and `syncEntryEditability()` flips every field when the width crosses 768px (the same `matchMedia` change listener as the inert handling). An earlier version left them editable and cancelled `pointerdown` instead. Chromium honors that and iOS Safari does not: WebKit focused the field from the touch, and once the keyboard had come up every later tap edited in place, so the sheet opened once and never again. **Headless tests cannot catch this**, so do not bring back a cancel-the-event approach on the strength of a passing Playwright run. Discoverability: the toolbar hint swaps to phone wording (`#toolbar-hint-phone`, set from `updateMobileViewCount`: "Tap a title to edit, star, move, or delete it." once there are titles), entries get a tap highlight, and `pulseFirstEntryOnce()` rings the first title twice the first time Preview opens with titles (localStorage `booklister.editHintSeen`; skipped under reduced motion and in the tour). An iPhone in landscape is wider than 768px and gets the desktop layout, inline editing included.
- **Tap-to-edit sheet is the phone's only per-title control surface.** `openBookEditSheet` / `#book-edit-modal` (a tap on a title's text opens it; it predates this layout) carries, above the text fields, **Star for cover**, **Up / Down** with the position ("3 of 13"), **Draft description** (same gate as the desktop Magic button: `LIBRARY_CONFIG && !disableAutodrafter`) and **Delete**. The desktop controls beside each title (`.list-item-controls`) are `display: none` on phones, since at phone zoom they rendered a few pixels across. Every sheet button calls the function the desktop control calls, so undo groups, autosave, Folio reactions and cover regeneration match: Star goes through **`toggleBookStar(bookItem)`**, extracted from the star button's former inline closure and now shared by both (with `getStarLimitState(bookItem)` recomputing the collage limit on every call); Up / Down call `handleBookReorder(i, i ± 1)`, the same path the drag handle's arrow keys use; Draft commits typed edits through `saveBookEditSheet` first, then calls `handleMagicButtonClick(book, { shiftKey: false })`; Delete calls `handleDeleteBook` after closing WITHOUT saving (it swaps a new blank object into the slot, so a later Save would write to an orphan) and asks for a second tap first ("Tap again to delete", reverts after 4s), because on a phone it is a large button a thumb can hit in passing and undo sits in the collapsed zoom bar. Star and Move keep the sheet open and refresh it via `updateBookEditActions()`. The sheet focuses its first ENABLED action on open, not the title field, so the keyboard doesn't cover the buttons before the user chooses to type. `.book-edit-action[hidden]` restates `display: none` because the base `.btn` rule's `inline-flex` otherwise beats Draft's `hidden` attribute (the "Known CSS Pitfalls" trap, hit in testing).
...
- Honeycomb / Jigsaw: no fixed grid per count. The cell size (Honeycomb) and row count (Jigsaw) are searched per list so every title gets one whole cell or piece; see Key Functional Area 4
...
### State Persistence
- `serializeState()` writes `ui.jigsawSeed` (the Jigsaw cut; `applyState` defaults a missing value to 1, so older saves get the default cut)
- `serializeState()` writes `ui.honeycombBackdrop` (`'soft' | 'sharp' | 'color'`; `applyState` maps the older `'bar'` to `'color'` and falls back to `'soft'` for a missing or unknown value), `ui.honeycombFill` (`{match, color, gradient, color2, direction}`; missing means match the title bar) and `ui.honeycombCoverSizePct` (50 to 110; missing means 100). Tilted's `ui.tiltCoverSizePct` shares the 50 to 110 range
...
- **Never shown on phones** (the 768px phone layout): `initToggle()` keeps `.folio-hidden` there whatever `folio-hidden` says in localStorage, app.js skips its page-load greeting at that width, the tour skips its force-show, and the More menu has no Folio item. At phone width he covered the corner of whichever view was open, caught taps there, and his nowrap bubble started ~74px off the left edge. Keeping the hidden CLASS (not a CSS `display: none`) preserves "hidden = actually off" below. The stored preference is untouched, so the same browser at desktop width still honors it after a reload. The tour still narrates as Folio from its panel's avatar.
...
- **Tail is procedural, not a CSS-rotated path**: folio.js rebuilds `#tail`'s outline and its three stripe paths every frame from a 13-point spine (`TAIL_REST`, fitted to the original drawing's centerline), each joint a damped spring chasing its parent. The CSS `#tail` state animations and reactions (idle sway, sleep droop to -70deg, startle, tail-swish) are **unchanged and still own the pose**: they rotate the whole group, and the springs feel that rotation, so the tip lags and overshoots. On top of that, a base-to-tip travelling wave sized per state (`TAIL_MOODS`), random tip flicks while idle/evaluating/sleeping, and short reaction moods via `tailReact()` (flatten → lashing, satisfied → contented curl, startle → the cartoon startle below). **Each mood is a pose, not just a speed** (`TAIL_MOODS`: `lean`, `bend`, `curl`, `sweep`, `quiver`, `puff`, `bristle`, `twitch`, `stiff`), because a cat says most of what it means with where the tail is: happy moods (excited, greeting, petted) stand tall and straight with a hooked tip and a small base quiver; worried drops low to his side; searching is lower and still with a twitching tip; the annoyed lash is a wide, low thrash, clearly bigger than anything happy. An earlier set varied only the sway around one upright pose, so excited and greeting wagged side to side like a dog, which in a cat reads as irritation, and looked much like the lash. The CSS `excitedTail` / `greetTail` keyframes are deliberately just a 1.5deg bob for the same reason; don't restore a sideways wag there. **The startle** (`TAIL_MOODS.startle`, held for `TAIL_STARTLE_MS`) shoots the tail straight up, rigid and bristled (about 1.65x the width, less than the first 1.9x because the fur locks add their own silhouette) and trembling, then relaxes back. **The bristle is curved fur locks, not teeth** (`bristledTailPath`): a first version drew a uniform zigzag that read as a serrated knife edge with a smooth cap stuck on top. Each lock is two quadratics, a full convex back on the side nearer the base and a short concave return, with its point swept toward the tail's tip, so the fur reads as lying the way raised fur lies. The left side, the rounded cap and the right side are each resampled by their own arc length (10 / 4 / 10 locks), so locks neither bunch on the inside of a bend nor stretch on the outside, and the counts are fixed so none pops in or out as the tail moves. Heights carry a fixed per-lock irregularity (a hash of the index, so no flicker), a slight ruffle, and fade to nothing over the first 2.5 segments where the tail meets the body. Lock height goes with `bristle` SQUARED: on the linear fade the settling fur spent most of a second as low wide bumps that looked lumpy, not calm. At zero height the locks lie flat along the outline, which is why the switch back to the smooth path (`TAIL_BRISTLE_MIN`) is invisible. It blends in with `TAIL_SNAP_TAU` instead of the usual `TAIL_BLEND_TAU`, and `stiff: 8` makes the springs stiff and critically damped for its duration: at normal stiffness the follow-through whipped the tip sideways and took half a second to settle, so it rose rather than snapped. There is no CSS `#tail` rule for `react-startle` any more, so it plays from any pose; waking from the sleep droop, the base `#tail` transition snaps the group up. Startle plays on waking, on a search that loses the connection, on a collage that fails to draw, and on a failed PDF export (`pdf-failed`), each followed by the worried state. `TAIL_MAX_LAG` caps how far a joint trails its target; without it a sharp swish kinked the outline and could fold the hook over itself. The rAF loop runs only while he is visible, the tab is foregrounded and reduced motion is off (a MutationObserver on the container's class restarts it, which covers the tour's direct class removal); otherwise the procedural rest pose is drawn once. The static paths in index.html are the no-JS fallback. To see the motion in a headless run, sample the path's `d` plus the group's computed transform over time and overlay them (onion skin); single screenshots hide whether it is fluid.
- **The drawing is a placeholder, not the goal.** The owner's vision: **a simple but elegant cartoon cat that moves like a cat and is a good work buddy.** The current drawing (the original, plus upright rounded-tip ears borrowed from a later redesign) is kept because nothing better has been found yet, not because it meets that bar. In September 2026 six redesigns were tried and rolled back: a detailed redraw (irises, whiskers, gradient fur) that read as unsettling; a shrunken head on a pear body, triangular; a round body wider than the head, bottom-heavy; a slim body narrower than the head; a from-scratch three-quarter cartoon cat; and a head-on version of it with thinner body and various leg slopes. The ears were the one part kept. Afterwards the original body was narrowed to 86% of its width about x=250 (body, stripes, bib and back paws, so the back paws sit closer to the front paws), and the tail moved 16px right to stay rooted inside the narrower side: the static tail paths, `TAIL_REST` in folio.js (base now (153,416)) and the `#tail` transform-origin in folio.css (146px 420px) move together, or the tail's root shows outside the body. **Lessons, so the next attempt does not repeat them.** (1) The brief holds a real tension: cuteness wants a big round head and a short body, elegance wants long lines (back, neck, tail). Every rejected version was an unnamed compromise between the two; name the trade-off with the owner before drawing. (2) Nudging SVG coordinates and screenshotting refines details on a silhouette nobody chose. Choose the silhouette first: several very different flat, faceless shapes on one page at real on-screen size, the owner picks one, and only then does detail go on. (3) Motion was the one thing that landed without reservation (the procedural tail). Moving like a cat is timing and weight, not drawing, and the current rig can do more of it: ear swivels toward events, weight shifts before moves, slow blinks with the head, settling into a loaf, grooming after a pet. Being a work buddy is behavioral too: settling near the user and keeping company rather than performing. (4) If the look matters enough, an illustrator may get there faster than coordinate edits. Ask before starting another redesign. **Every animation targets only the named groups** (`#head-group`, `.ear-left-anim` / `.ear-right-anim`, `#eyes`, `#nose`, `#mouth`, `#glasses`, `#body`, `#front-legs`, `#tail`) and their transform origins, so if the owner does ask for shape changes, shapes inside them can change freely as long as those ids and origins stay put. (A regex over `d="..."` attributes once also matched `id="tail"` and renamed the group to `t a i l`, silently killing every tail animation; match `\bd="` when rewriting paths, or replace whole blocks.)
- **Only the cat takes the pointer**: `#folio-scene` and the SVG are `pointer-events: none`, and just the cat's groups (`#tail`, `#body`, `#back-feet`, `#front-legs`, `#head-group`) opt back in with `visiblePainted`. The scene is a 760x459 rectangle the cat fills well under half of, and a scene-wide hit area made the empty air above the shelves an invisible Folio button over the controls behind it (the branding uploader, among others). Clicks bubble to `#folio` as before. Petting is tracked on `document` instead, gated on the pointer having been over the cat within `PET_OFF_CAT_MS` (450ms): a real rub swings past his outline at each end, which is where the stroke-counting reversal happens, and an SVG-only listener lost those reversals and reset on every overshoot. Shelf and books are pass-through scenery. A new part of the cat drawn outside those five groups needs adding to that rule or it will not be clickable.
- **Speech bubble**: anchored by its bottom edge (multi-line quips grow upward), wraps at `min(250px, 70vw)` with `text-wrap: balance`, and scales from its pointer. Words fade in one at a time (`renderBubbleText`, 55ms stagger capped at 900ms; the whole sentence is in the DOM at once so `role="status"` announces it once), and the hold clock starts after the last word lands. A line replacing a visible one squashes in place (`.swap`) instead of collapsing and re-popping. The 250ms cleanup after the shrink animation is tracked as `bubbleHideTimer` and cancelled by `showBubbleNow` / `silenceBubble`; untracked, a line shown during a shrink was wiped a fraction of a second after it appeared. `*stage directions*` render muted via `.speech-action`. While a spoken line arrives, `#folio-scene.talking` flaps `#mouth-open` (an ellipse inside `#mouth`); the class lives on the scene, not the SVG, because `setState` rewrites the SVG's class list. No talking for pure stage directions, while sleeping, or during the tour (tour.css hides the bubble there, so the mouth would move with no words). **Sizing and placement live in `fitBubble()`**, run on every line and again on the sidebar's slide, a resize and a webfont load: (1) two-line bubbles are shrink-wrapped to the widest line, measured from the word spans (`text-wrap: balance` evens the lines but the box kept its full width, leaving 40-80px of empty space), and the fit repeats on `document.fonts` `loadingdone` because the greeting can arrive before Caveat does; (2) the bubble is shifted sideways just enough to stay on screen and clear of the sidebar (with the sidebar collapsed the scene sits at x=0 and a centred bubble ran up to 53px off the left edge), via `--bub-dx`, while `--bub-tail` moves the pointer and the pop's origin back over his head. **Every bubble transform, including the reduced-motion ones, must include `var(--bub-dx)`**, or the shift is lost mid-animation. The bubble sits at `bottom: 95%` of the scene, a share rather than a fixed offset, because the scene scales and a fixed 10px sat on his ear tips at small sizes. A swap whose width changes by more than 40% pops in fresh instead of squashing. The edge is a two-layer `filter: drop-shadow`, not `box-shadow`, so the pointer is outlined too (the white bubble sits over white panels). When a line finishes and nothing follows, the cleanup also empties the text, so screen readers can't find a stale line in the invisible bubble.
- **Micro-reactions**: nod, perk, wince, watch (eye tracking), yawn, startle, satisfied, squish (tactile press-down on clicks 1-3 of the escalation ladder; re-triggering force-restarts it via a style flush — the sanctioned exception to the same-frame no-restart rule — so EVERY click visibly presses him down; nod remains the reaction for app-event triggers like star toggles), flatten (exhausted 2s sag on the click-spam tier — re-triggering deliberately does NOT restart it, so sustained spam reads as him staying slumped; each spam click instead pops a 💢 grump mark, the pestered-tier counterpart to the petting hearts, so every click still visibly lands), plus three idle fidgets (stretch, ear-flick, tail-swish). Every reaction survives `setState`: it rewrites the SVG's class list, so it re-adds `react-<activeReaction>`; before that, `celebrate()`'s state change 300ms in cut every nod and perk short.
- **Idle fidgets**: every 14-32s of idle, a randomized scheduler plays one fidget from a shuffle bag so the idle loop doesn't read as a loop. Gated on: visible, idle state, unguarded, no reaction in flight, not eye-tracking a drag, tab foregrounded, and no `prefers-reduced-motion`.
- **TODO, revisit: a paw-licking groom fidget.** Built and rolled back in September 2026 (six commits on `claude/brave-ptolemy-tdtooj`, reverted as one block; `git log` for "Folio groom" has the full code). The owner wants to come back to it. What it did: he shifts his weight, lifts his right front paw to his mouth, shuts his eyes and licks it in two bursts of three while a tongue flicks out of an open mouth, then sets the paw down. What was learned, so the next attempt starts ahead: (1) Rotating the whole resting leg about its top reads as a gate arm, and swapping paw drawings mid-swing pops. (2) The version that got furthest drew the arm in JS every frame inside the tail's rAF loop: a shoulder-elbow-wrist spine stroked twice (dark at width+8, orange at width, round joins), which gives a clean round elbow at any bend with no seams; the raised elbow folded in under the shoulder so the upper arm hid inside the forearm (an elbow out at his side read as a U-shaped pipe); the paw was one outline morphing from the standing mitten into a raised bean wider than the wrist (a paw narrower than the arm reads as a fingernail); the wrist target was read from `#head-group`'s live transform so the tongue met the paw during head bobs. (3) A stroke cannot taper, so the drawn leg stayed on screen at rest and handed over once the paw moved. (4) Still unsolved when it was paused: early in the lift the forearm folds toward the viewer and the paw sits in a little cup of arm, and the whole thing never quite read as a cat rather than a well-rigged puppet. That last point is the one to start from, per lesson (3) of the drawing note above: timing and weight before geometry.
- **Quip system**: Triggered quips for specific events (e.g., 'book-added', 'pdf-exported') + ambient shuffle-bag pool (prevents repeats). Bubble precedence follows a **physical-vs-standard grammar** (the `bubblePhysical` flag in folio.js). PHYSICAL lines — purr/pet-interrupt/mollified from petting, poke-interrupt/annoyed/pestered from clicking — interrupt STANDARD lines (ambient/context/triggered/greeting) instantly via `interruptBubble()`. Physical lines never stomp each other mid-read: a new tier line waits in the one-shot follow-up slot (`queueTierLine`) and shows after the current hold completes (each click's draw overwrites the slot, so escalation upgrades the waiting line). A deferred standard quip that comes due while a physical line is fresh is dropped, not shown, and so is one that comes due after he was hidden (the tour's exit re-hides him). **Pacing**: a line can't be replaced until it has had `bubbleMinVisibleMs`, which scales with length (1.5s floor, 3s cap; a flat 1.5s cut 56-character lines off unread), and a line in its 250ms shrink still counts as up, so it can't wipe a queued follow-up. **Repeats**: `setState` passes a `state:event` key, and the same event firing again while its line is fully up is skipped (three Quick Add batches used to chain three lines about the same act). A lone click's quip waits `SINGLE_CLICK_QUIP_DELAY_MS` (260ms) so the first click of a double-click doesn't flash a line into the live region before the annoyed line; the squish stays instant. The wake sequence says "I'm awake!" only into silence: if whatever woke him has already spoken, the lines would otherwise play in reverse. A lone click on a fresh (<2.2s) standard line breaks him off mid-sentence with a poke-acknowledgment line (`clickInterruptQuips`); on a stale one it just shows the next quip. Clicks and pets both END any active guard window — the guard blocks cascading app hooks, not the user's hand.
- **Context awareness**: `app.js` `init()` registers a provider via `window.folio.setContextProvider(fn)` returning live facts (`bookCount`, `maxBooks` — the *dynamic* max, 13-15 depending on QR/branding toggles — `starredCovers`, `requiredCovers`, `listName`). folio.js pulls through this single function and never touches app internals (same one-way-window spirit as the Firebase layer's `window.*` globals). Context quips fire on the single-click path (~60% when a rule matches, else ambient pool); rules live in `contextQuips` in folio.js. Within `INTERACTION_QUIET_MS` (6s) of a pet or annoyed-tier click, a lone click stays squish-only — no context/ambient quip — so shelf musings can't stomp a purr mid-read or land as a non-sequitur right after a spam burst (lone clicks don't extend the window, keeping the solicit-a-comment affordance). The **collage-ready milestone** (star that crosses the required-covers threshold) is detected in app.js's star-toggle handler (before/after `hasEnoughCoversForCollage` check) and fires a `collage-ready` celebrate instead of the plain nod. The `collage-generated` line plays **only on a Create Cover press** (`generateCoverCollage({ announce: true })`); every other caller is an automatic rebuild, and praising those repeated one line six times in twenty seconds. Other events are worded to the app state: `covers-needed` (not enough stars) vs `cover-images-needed` (stars without images), `collage-failed` (a drawing error, formerly voiced as a network problem), a one-row paste fires `book-added` rather than the "stack" lines, and `celebrateIfSlotsFull()` in app.js gives search-add and both Quick Add paths the same `slots-full` beat. "Clear content, keep styles" gets a perk plus the idle `content-cleared` pool. The sleeping dream lines rotate on the `inactivity` event (the old ambient pool was unreachable, since clicking a sleeping cat wakes him).
- **Memory**: light localStorage persistence, keys `folio-visits`, `folio-last-visit`, `folio-first-book-seen` (same naming convention as `folio-hidden`). Visit count + gap drive returning-visitor and time-of-day greeting overrides on `page-load` / `draft-restored`; first-ever `book-added` gets a one-time milestone line (only when the list truly has one title — a veteran toggling Folio on mid-list burns the flag silently). Memory only ticks while Folio is visible: an invisible cat doesn't collect memories.
- **Petting**: rub the pointer back and forth over the cat (3 completed strokes ≥15px within 2s, 8s cooldown) → satisfied slow-blink + floating hearts + purr quip. Stroke extent accumulates per direction so slow gentle rubs register. Pointer events + `touch-action: pan-y` on the SVG make finger-rubs work on touch without breaking vertical page scroll. Suppressed while drag-watching or mid-wake; a rub during a guard window instead ENDS the guard early — the guard blocks cascading app hooks, not the user's hand (pet-during-entrance-greeting was the classic dead spot before this). Sustained rubbing between purr quips stays alive via `continuePet()`: a throttled heart per completed stroke plus the satisfied squint refreshed, with a fresh quip only after the 8s cooldown. A rub that lands while he's mid-sentence interrupts the bubble **immediately** via `interruptBubble()` (bypassing the MIN_VISIBLE_MS pacing queue — a deliberate physical act outranks queued async quips; clicks share this treatment per the quip-system grammar above) and swaps in a line from the interrupted-thought pool ("—oh. Oh, that's the spot."). He never resumes the interrupted line; that's intentional — losing the train of thought is the joke, and several lines lampshade it. Don't add resume logic. **Anger overcomes petting — forgiveness is earned in two stages**: while the pestered grudge is hot (`PESTERED_MEMORY_MS`, 6s, re-armed by every spam click), the FIRST rub is rebuffed — his angry line stands un-interrupted, he answers with a dismissive tail-swish + grump mark (no hearts, no squint), and a rebuff line ("A rub doesn't undo all that clicking." / "I'm still mad. ...keep going, though.") answers the hand once his current line has had its minimum time (`showRebuffLine`), taking the place of any grievance still in the follow-up slot. **The sulk clock starts when the rebuff is on screen, and a started rebuff keeps the grudge alive until forgiveness** (`angerIsHot`): timed from the rub, the rebuff waited out a full hold while the sulk and the 6s grudge expired, so it arrived after forgiveness or never. Rubbing on through the sulk (`REBUFF_HOLD_MS`, 2.5s; attempts mid-sulk get cold-shoulder grump marks) earns the grudging-forgiveness pool ("...fine. You're forgiven."), which clears the grudge, may cut off an angry line mid-read (the emotional turn justifies the one stomp), and drops any queued grumpy follow-up. Outside a grudge, pet joy is instant (squint + hearts) but the purr LINE follows the bubble grammar: it queues behind his own fresh physical line (e.g. a mild annoyed line), breaks off standard lines with the interrupted-thought pool, and purrs plainly into silence.
- **Guard system**: `folio.guard(duration)` suppresses setState/react to prevent cascade during state restoration
- **Hidden = actually off**: while `.folio-hidden` is set, bubbles are suppressed in JS (no ghost `aria-live` announcements), the inactivity/sleep clock doesn't run, fidgets skip, and CSS pauses all SVG animations + flips `visibility: hidden` after the fade (drops the scene from the a11y tree and stops paint work). Toggling him off also calls `silenceBubble()` + `clearReaction()`, dropping the live line, its text, every queued line (pacing defer + follow-up slot) and any in-flight reaction — so summoning him back never resurrects a sentence or a half-played squish from the previous session.
- **Accessibility**: the speech bubble is `role="status"` / `aria-live="polite"`; bubble hold time scales with text length (3.2s floor, 6.5s cap). `prefers-reduced-motion` freezes every SVG animation and the hearts via the CSS block appended at the end of folio.css (double-id + `!important` to outrank the reaction rules' own `!important`); the bubble still appears/disappears, just without the pop/shrink motion.
- **API**: `window.folio.setState()`, `.react()`, `.celebrate()`, `.showBubble()`, `.clickFolio()`, `.guard()`, `.setContextProvider()`
...
- **`privacy.html`** — privacy policy with CalOPPA-compliant disclosures: effective date, Do Not Track section, third-party collection disclosure, review/correct/delete process. Voice matches the other content pages. No em dashes (preference). If the policy text ever changes, update both "Effective" and "Last updated" dates at the top of the page.
  - **TODO (on or after October 25, 2026): delete the `<p class="content-update-note">` callout** below the h1. It announces the September 25, 2026 review, which added disclosures the page had been missing (GitHub Pages hosting and its visitor IP logs, placehold.co placeholder images, pasted cover links, the extension's two synced settings) and corrected the Firebase sign-in description (a token in local storage, no cookie). The page promises material-change notices stay visible "for at least one month after the update", so it comes down once the month is up. (The previous callout, for the August 11, 2026 addition of Cloudflare Web Analytics, was removed in that same review.) Delete the markup only; leave the `.content-article .content-update-note` rule in `styles.css` for the next one. There is no changelog on the page today, so removing the callout leaves the two dates at the top as the only record that anything changed; a permanent changelog under "Changes to this page" was discussed and deferred, and is the natural thing to add if this comes up again.
...
**Analytics**: All six content pages, plus `index.html`, load `assets/js/analytics.js` from the `<head>` (cookieless Cloudflare Web Analytics, self-gated to the public production host). A new content page should carry the same line. `admin/index.html` deliberately does not.

**No Font Awesome stylesheet on content pages**: the six content pages do NOT load the Font Awesome CSS. They used to, purely to render the one book-open glyph in the header logo, which meant every visit pulled ~100KB of render-blocking CSS plus a webfont for a single decorative icon (it showed up as the worst LCP on the *smallest* page). The logo mark is now that **same Font Awesome 6.4.0 book-open path, inlined** as `<svg class="logo-icon">`, styled by `.app-header .logo .logo-icon` in `styles.css`. `index.html` and `admin/index.html` still load the Font Awesome stylesheet and still use `<i class="fa-solid ...">`, because they genuinely use many icons.

**Attribution**: Font Awesome Free icons are CC BY 4.0. When loaded as the CDN stylesheet (index.html, admin/index.html) the licence notice ships inside `all.min.css`, so nothing extra is needed there. Inlining a path takes the icon out of that file, so **the attribution comment Font Awesome ships inside its own SVG is kept in the markup immediately before the `<svg>`** on each content page. Keep that comment when copying the logo to a new page, and keep it if the icon is ever swapped for another Font Awesome one. If the path is ever replaced with original artwork, the comment goes with it.

**Sizing note**: the icon's viewBox is `0 0 576 512`, so the CSS pins `height: 1.2rem; width: auto` rather than setting both. That reproduces how Font Awesome sizes the glyph (height = 1em, width follows the aspect ratio) and renders 22x19 px, matching the `<i>` it replaced. A square viewBox icon would need different rules.
...
**Site-wide footer nav**: A 32px dark slate footer at the bottom of every page (including `index.html`) contains the shared nav: Home · About · For Libraries · Helper · Privacy · Terms · Contact. The tool page's `app-container` height is `calc(100vh - 52px - 32px)` to make room for this footer. Folio and the zoom controls are positioned `bottom: 32px` to sit above it. On phones (768px and below) the footer is hidden on every page; content pages move its links into the header's `.content-menu` (see Phone Layout). The footer is hidden in `print-mode` so it doesn't leak into PDF exports (which html2canvas captures from `#print-page-1`/`#print-page-2` directly anyway).
...
3. **Folio state timeout pattern** — mostly cleaned up: `folio.celebrate(opts)` consolidated the old copy-pasted `setTimeout` triples in app.js (reaction → state+quip → return to idle, with a rolling return timer so back-to-back celebrations don't stomp each other), and the wake sequence in folio.js was extracted into `wakeUp()`. (The tail used to be moved in the DOM between in front of and behind the books; it now always paints behind them, set by its position in index.html.) When adding a new Folio trigger in app.js, call `celebrate()` — don't hand-roll new setTimeout chains.
...
**The same trap, without `[hidden]`: bare class rules lose to existing element-scoped rules.** `admin.css` has `.admin-section p { margin: 0 0 16px; color: var(--admin-text) }` at `(0,1,1)`. A new `.admin-legal-note { margin: 24px 0 0; color: var(--admin-text-light) }` is only `(0,1,0)`, so **both** declarations were silently reverted — the note rendered in body-text black, flush against the button above it, with no error anywhere. The fix was scoping to `#admin-signin .admin-legal-note`. Before adding a class rule for a new `<p>`, `<ul>`, `<a>`, or `<h*>` inside `.admin-section`, `.content-article`, or `.modal-content`, check whether that container already styles the bare element — those descendant rules outrank a plain class. Verify with `getComputedStyle`, not by eye: a wrong-but-plausible color or margin looks like a design choice, not a bug.

### Focus rings: `:focus-visible` on buttons, `:focus` on text fields

The shared ring rule in `styles.css` (`.tab-btn`, `.btn`, `.btn-header`,
`.btn-header-primary`, `.add-to-list-button`) uses **`:focus-visible`**, not
`:focus`. A button is an action; a ring that persists after a mouse click reads
as a selected mode rather than as "your keyboard is here". Keyboard focus still
rings, which is the case the indicator exists for. Text inputs and
...
switch, so any engine without `:focus-visible` was already dropping the whole
block and showing no ring on any of these buttons. Moving the rest to
`:focus-visible` therefore cannot cost support the rule did not already require.
The same logic applies to any future rule here: do not mix a `:focus-visible`
selector into a group you need working on an engine that lacks it.

The icon-button rules (`.delete-button:focus`, `.magic-button:focus`,
`.star-button:focus`, `.carousel-button:focus`, `.drag-handle:focus`) still use
plain `:focus` and were left alone. The same argument applies to them; they were
simply out of scope when the buttons were changed.
...
**The repeat offender**: `.font-select`, and it has **two** class-driven consumers, not one.

1. `populateFontSelects()` queries every element with this class and **overwrites its options** with the FONTS list. The bug presents as a dropdown that should show (e.g.) "Top to bottom / Left to right" but shows "Arial / Calibri / Cambria" instead. Fix: add the id to the `:not(...)` allowlist in that selector.
2. The `createCustomFontDropdown()` sweep inside `initializeCustomFontDropdowns()` queries `.export-controls .form-group[data-style-group]:not([data-style-group="qr"]) .font-select` and **replaces each match with the custom dropdown widget**, which hides the native select at 1px wide. An allowlist entry in `populateFontSelects()` does nothing about this one. The bug presents as a select that computes to `width: 1px` with its `flex-grow` apparently ignored.

**For a non-font select inside a `[data-style-group]` box, do not use the class at all.** Both traps disappear, and nothing is lost: the generic `input[type="text"], input[type="number"], select, textarea` rule in `styles.css` already supplies the border, padding, radius and font size, while `.font-select` itself only adds `flex-grow: 2`. `#byline-prefix-select` is the worked example; it was built with the class, hit trap 2 in browser testing, and dropped it.
...
- `#byline-prefix-select` (byline word picker) — hit BOTH consumers, because unlike the three above it sits inside a `[data-style-group]` box. Fixed by dropping the class rather than by allowlisting.
...
| `CONFIG.LOOKS` catalog | `tests/config.test.js` (structure suite: fonts must exist in FONTS, layouts/positions/colors validated; its `LAYOUTS` list deliberately omits `honeycomb` and `jigsaw`, and `renderLookCoverCanvas` has no case for them, so a look using either needs both extended first), `app.js` applyLook/buildLookTitleStyles (consume the schema), `BookUtils.pickFeaturedLooks` (reads `months`). A look's `coverTitle.simple`/`lines` shapes mirror `serializeState()`'s `styles.coverTitle` — if that schema changes, looks change with it |
| `CONFIG.BYLINE_PREFIXES` | `book-utils.js` (`getBylinePrefixWords`/`getBylineOpenerWords`/`stripBylinePrefix`/`setBylinePrefix` all read it), `app.js` `initBylinePrefixSelect()` (one option per entry) and `getAiDescription`'s author parse (strips the same words), `tests/config.test.js` (shape, no-duplicates, and the surname-particle guard on `opener`). Adding a word ships a new option and teaches the drafter parse in the same commit, which is the whole reason it is one list. **Decide `opener` deliberately**: on means the word may be matched mid-line as the tail of `Edited by` / `Escrito por`, and a surname particle with it on deletes the author's first name |
...
| BooklistApp public API | `tour.js` (calls `enterTourMode`, `exitTourMode`, `applyState`, `generateCoverCollage`, `updateBackCoverVisibility`, `resetZoom`, `fitToWidth`, `showMobileViewFor`) |
| Header buttons in `index.html` (`.header-actions`) | `MOBILE_MENU_CONTROLS` in `app.js`: phones hide every header control except Generate PDF and More, so a new header button needs a menu entry to be reachable there. A tour step targeting a header control resolves to the More button on phones (`resolveStepTarget` in `tour.js`); give it a `phoneText`. See "Phone Layout" |
...
`PDF_CANVAS_SCALE` 6.25 means 6600x5100 = 33.7M pixels per page, and iOS Safari
capped canvas area at 16.7M, failing blank rather than erroring. **That cap is out
of date for current iPhones** (researched September 2026): WebKit's `maxCanvasArea`
in `Source/WebCore/html/CanvasBase.cpp` is now `8192 * 8192` = 67.1M px on
`PLATFORM(IOS_FAMILY)` (reportedly since iOS 18.0) and `16384 * 16384` elsewhere,
and the separate total-canvas-memory cap was deleted outright (commit 265628@main,
WebKit bug 195325). iOS 17 and earlier still hit 16.7M, and a low-memory phone can
still reload the tab under two ~135 MB page canvases. Android was not checked.
`exportPdf` now refuses to save a blank capture (see Key Functional Area 5), so a
device that fails says so instead of producing an empty PDF. Rendering in a
*viewer's* browser still means every visitor's device, including old ones.
...
6. **Analytics run on the public production host only, and the privacy policy is the spec.** `analytics.js` injects the Cloudflare Web Analytics beacon (cookieless: no cookies, no localStorage, no fingerprint, no visitor ID, no retained IP) and self-gates to `booklister.org` / `www.booklister.org`. It must never run on a branded library subdomain: `privacy.html` promises libraries that their instances carry no usage analytics and no telemetry, and that gate is the only thing making the promise true. It also stays off `*.pages.dev` and localhost so dev traffic doesn't pollute production numbers, and off `admin/` entirely. **The "Analytics" section of `privacy.html` is a user-facing description of exactly what this file loads** — if you change the beacon, add a second analytics provider, widen the host gate, or start collecting anything event-shaped, that section, the "Cookies and tracking" section, the "Do Not Track" section, and the CCPA paragraph all have to change in the same commit, along with both dates at the top of the page. Adding analytics to a branded instance is not a code change, it is a policy change: talk to the library first. Do NOT treat the beacon as free to sprinkle onto new pages — every page carrying it is a page whose visits are counted, and the privacy policy enumerates the scope.
