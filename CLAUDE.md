...
**No Font Awesome on content pages**: the six content pages do NOT load the Font Awesome stylesheet. They used to, purely to render the one book-open glyph in the header logo, which meant every visit pulled ~100KB of render-blocking CSS plus a webfont for a single decorative icon (it showed up as the worst LCP on the *smallest* page). The logo mark is now an inline `<svg class="logo-icon">` styled by `.app-header .logo .logo-icon` in `styles.css`, sized in rem to match the glyph it replaced. `index.html` and `admin/index.html` still load Font Awesome and still use `<i class="fa-solid ...">`, because they genuinely use many icons. **A new content page should copy the inline SVG from an existing one, not add the Font Awesome link back.**
