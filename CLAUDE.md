...
**TSV output format**: `title<TAB>author<TAB>callNumber<TAB>coverUrl\n`. Title combines `brief.title` + `: ` + `brief.subTitle` when subtitle is present (so the post-colon word triggers Booklister's subtitle-capitalization rule). Author goes through `cleanAuthor()` which strips trailing lifetime-date suffixes (`"Styron, William, 1925-2006"` → `"Styron, William"`) so Booklister's `flipAuthorName` does the right thing on add. The 4th `coverUrl` column is `entities.catalogBibs.<bibId>.brief.coverImage.large` from the SSR state — a Syndetics URL the parent BiblioCommons site already serves. Booklister stores it into `customCoverData` on add, so the book renders with its real cover instead of the placeholder. Empty when the bib has no `coverImage` block. Embedded tabs / newlines in any field are collapsed to spaces.
