...
- **The drawing (redrawn from scratch, September 2026).** After four rejected revisions of the old drawing (a detailed redraw that read as unsettling, a shrunken head on a pear body that read as triangular, a round body wider than the head that read as bottom-heavy, and a narrow front-view body the owner still found off), Folio was redrawn from scratch after the owner's reference images: **a flat cartoon cat sitting and facing the reader head on** (a three-quarter version came first; the owner preferred head on and found its legs too basic and its back legs unreadable). A big wide head, flatter on top and fuller at the cheeks; upright ears with rounded tips; small dark oval eyes with one highlight behind his round glasses; a tiny nose and a "ω" mouth; two whiskers a side poking past the cheeks; a cream lower face; three forehead stripes. The body is a symmetric sitting shape narrower than the head, with a cream chest bib. Two slim front legs run down the middle to cream-tipped paws with a toe line each; their outer edges start wide at the shoulders and angle in toward the paws (the owner found straight columns blocky, and the reverse slope, narrow at the top, read wrong too), while the inner edges stay straight and meet in the middle. The back legs read as a thigh curve on each side ending in a small cream back paw beside the front paws; those back paws live in `#back-feet`, which is no longer empty. A thick even outline (5-5.5) and flat fills, no gradients. Keep this style: adjust proportions and poses, do not add rendering detail. **The tail's anchor**: `TAIL_REST` starts at (182,416), inside the body's left side, and the `#tail` transform-origin in folio.css is 175px 420px to match; `TAIL_LEAN_DEG` (-14) rotates the resting spine outward so the tail clears the head, and the sleep droop / sleep curl / startle keyframes are reduced by the same 14deg so the sleeping tail lies along the shelf instead of dipping below it. Move all four together or the tail pokes out from under the body or clips the shelf. **Every animation targets only the named groups** (`#head-group`, `.ear-left-anim` / `.ear-right-anim`, `#eyes`, `#nose`, `#mouth`, `#glasses`, `#body`, `#front-legs`, `#tail`) and their transform origins (head 250,310; ears 170,150 / 330,150; nose 250,240; mouth 250,256), so shapes inside them can change freely as long as those ids, the origins, the glasses rings staying centred on the eyes (200,212) / (300,212), and the tail's `fur-orange` + `stripe` classes stay put. (A regex over `d="..."` attributes once also matched `id="tail"` and renamed the group to `t a i l`, silently killing every tail animation; match `\bd="` when rewriting paths, or replace whole blocks.)
