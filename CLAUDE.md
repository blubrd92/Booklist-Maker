...
- **Micro-reactions**: nod, perk, wince, watch (eye tracking), yawn, startle, satisfied, squish (tactile press-down on clicks 1-3 of the escalation ladder; re-triggering force-restarts it via a style flush — the sanctioned exception to the same-frame no-restart rule — so EVERY click visibly presses him down; nod remains the reaction for app-event triggers like star toggles), flatten (exhausted 2s sag on the click-spam tier — re-triggering deliberately does NOT restart it, so sustained spam reads as him staying slumped; each spam click instead pops a 💢 grump mark, the pestered-tier counterpart to the petting hearts, so every click still visibly lands), plus four idle fidgets (stretch, ear-flick, tail-swish, groom). **Groom** (3.4s) lifts his right front paw to his mouth, shuts his eyes and licks it: `#groom-leg` (the right leg, paw and toe notches, split out of `#front-legs` alongside `#front-leg-left`) rotates 152deg about the top of the leg (281,380) and shifts up so the paw lands beside his mouth, the head tilts and bobs toward it, `#mouth-open` scales open, and `#tongue` (a group inside `#mouth`: a pink U whose flat, unstroked top tucks into the open mouth, plus a centre crease; hidden by CSS except during this reaction, and tilted by a wrapper `<g>` because the CSS animation owns the group's own transform) flicks in and out. An earlier tongue was a fully outlined oval under a closed mouth, which read as a sticker on his chin; the open mouth and the open top are what make it come *out of* the mouth. The lifted paw swaps drawings mid-swing (at about 10% and 90%, while the leg moves fast enough that the swap reads as motion): `#groom-paw-rest` is the shelf paw, whose flat standing edge would face up once raised, and `#groom-paw-raised` is a mitten wider than the wrist (about 66 units against the leg's ~55 there) whose far end is a row of four toe bumps in the outline itself. A paw narrower than the arm read as a fingernail on the end of it; a cat's paw is the widest part of the foreleg. The swap is instant (keyframes at 9.99% / 10%): a crossfade left both paws half transparent and the chest patch showed through the arm. Toes drawn as straight interior lines on the raised paw read as a human fist's knuckles; the scalloped outline is what reads as a cat's paw. Both live inside `#groom-leg` in the leg's resting frame, so the leg's rotation carries whichever is showing. The paw pose and the tongue's tilt were tuned together at 6x zoom so the whole tongue shows and only its tip meets the toes (too far left and the paw swallows the mouth, too far right and there is a gap and he is just sticking his tongue out); the paw drawn in front hides whatever of the tongue it overlaps. **`#front-legs` is drawn after `#head-group`** so the raised paw passes in front of his chin; the legs never overlap the head at rest (head bottom ~315 even with every head animation's translateY, legs start at 380). The pose was placed so the leg's open, unstroked top end lands on orange fur rather than on the cream chest bib, where it would show as an edge; recheck that if the leg, bib or pose changes
