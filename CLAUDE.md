...
- **Quip system**: Triggered quips for specific events (e.g., 'book-added', 'pdf-exported') + ambient shuffle-bag pool (prevents repeats). Click-annoyance tier lines (annoyed/pestered) route through `queueTierLine`: if a bubble is up they wait in a one-shot follow-up slot and show only after the current hold fully completes (each click's draw overwrites the slot, so escalation upgrades the waiting line) — never stomping a line mid-read the way the 1.5s pacing defer would.
