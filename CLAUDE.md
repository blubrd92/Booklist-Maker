...
- **Byline words**: `BYLINE_PREFIXES` (By, Por, Par, Di, Ni, Von) — the words the Author box's byline picker writes AND the words `getAiDescription`'s author parse strips. Entries carry an optional `opener: true` permitting a mid-line match; **a word that doubles as a surname particle must not carry it**. One list on purpose; see Key Functional Area 6
