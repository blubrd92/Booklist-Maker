...
- `BookUtils.toSentenceCase(str)` - Lowercases the whole string, then capitalizes the first *letter* (not the first character, so `¿quien...?` and `«la ciudad...»` capitalize the real first word). For Spanish and other languages that capitalize only the first word and proper nouns in titles. Deliberately blunter than `toTitleCase`: no acronym preservation (`SPQR: a history` becomes `Spqr: a history`) and no capital after a colon (per RAE, a subtitle naming a partial aspect takes a colon plus lowercase). The tradeoff buys the thing that matters more here: a title arriving entirely in caps is actually fixed instead of being mistaken for a string of acronyms and passed through, which is what `toTitleCase` does to it. Interior proper nouns are flattened and need hand-fixing. Used by the Title Capitalization transform in the Text & Styling tab, NOT by Quick Add.
