...
```javascript
{
  ...
  featureInCollage: boolean,      // Showcase layout: render at double size. Only
                                  // meaningful while includeInCollage is true;
                                  // applyState clears it on unstarred books. Not
                                  // "content" for isDraftStateEffectivelyEmpty
                                  // purposes (a flag on a non-blank book — the
                                  // book itself already makes the draft non-empty)
...
4. **Cover Collage**: `generateCoverCollage()` renders starred books in 5 user-facing layouts (`data-layout` values in `index.html`'s layout picker): **Classic**, **Masonry**, **Staggered**, **Tilted**, **Showcase**. Title bar with 5 position options. **Internally there are 6 draw functions**, because the Classic layout has two visual variants gated on the "Show shelves" toggle: `drawLayoutClassic` (no shelves) and `drawLayoutBookshelf` (Classic geometry with horizontal shelves drawn behind the books). Plus `drawLayoutMasonry`, `drawLayoutStaggered`, `drawLayoutTilted`, and `drawLayoutShowcase`. The dispatch in `generateCoverCollage()` switches on `selectedLayout`, with the `classic` case branching on `showShelvesToggle` to pick between Classic and Bookshelf. So "Bookshelf" is not a separate user-facing layout — it is the Classic layout with shelves on.
   - **Showcase** is the only layout where covers render at different sizes: a mixed-size quilt of 1×1 unit covers plus a few 2×2 "showcased" covers. The unit cell keeps the 0.75 book aspect, so a 2×2 cover tiles exactly like four singles plus their gutters — no cropping, no holes (verified per pattern: every unit cell covered exactly once). Which covers go large is user-controlled via the per-book showcase toggle (`featureInCollage`, the `.showcase-button` rendered on starred book cards ONLY while the Showcase layout is selected); flagged books claim the 2×2 spots in collage order, unclaimed spots fall back to list order, surplus flagged books render standard size, and extra collage covers (13-20) are never showcased. The flags travel to the draw function as `layoutOptions.featuredFlags`, an array kept index-aligned with `images` (alignment is preserved even when some image loads fail — both arrays are built together in the load callback). Patterns live in `SHOWCASE_PATTERNS` in app.js: 12 covers = 5×6 units (6 doubles + 6 singles), 16 = 5×5 (3 doubles + 13 singles), 20 = 5×7 (5 doubles + 15 singles). The title bar inserts at a pattern segment boundary so it never splits a double; with only 3-4 boundaries per pattern, the center and lower title positions coincide at 12- and 16-count (20-count supports all five distinctly).
...
- Showcase: unit grids 12=5×6, 16=5×5, 20=5×7 (see the Showcase bullet under Key Functional Areas for the double/single split per count)
