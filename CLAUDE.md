...
- **The drawing: keep the simple style.** Three directions were tried in September 2026 and rejected by the owner: a detailed redraw (irises, whiskers, gradient fur, tabby markings) read as unsettling, a shrunken head on a pear body read as triangular, and a rounded body wider than the head read as bottom-heavy ("I wanted him to look catlike"). The target, from the owner's reference images, is a cute flat cartoon cat: the ORIGINAL full-size head, face and flat colors on an egg-shaped body **narrower than the head at every point**, long slim front legs running from under the chin to the shelf with small cream feet, cream chest showing between the legs, and no visible back feet (`#back-feet` is kept, empty, because the pointer-events rule and CSS still name it). Future changes should adjust proportions and poses, not add rendering detail. **The tail's anchor moved with the narrow body**: `TAIL_REST` starts at (182,416), inside the body's left haunch, and the `#tail` transform-origin in folio.css is 175px 420px to match; `TAIL_LEAN_DEG` (-14) rotates the resting spine outward so the tail clears the head, and the sleep droop / sleep curl / startle keyframes were reduced by the same 14deg so the sleeping tail still lies along the shelf instead of dipping below it. Move all four together or the tail pokes out from under the body or clips the shelf. **Every animation targets only the named groups** (`#head-group`, `.ear-left-anim` / `.ear-right-anim`, `#eyes`, `#nose`, `#mouth`, `#glasses`, `#body`, `#front-legs`, `#tail`) and their transform origins, so shapes inside them can change freely as long as those ids, the origins, and the tail's `fur-orange` + `stripe` classes stay put. (A regex over `d="..."` attributes once also matched `id="tail"` and renamed the group to `t a i l`, silently killing every tail animation; match `\bd="` when rewriting paths.)
