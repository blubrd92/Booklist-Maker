...
- **Hidden = actually off**: while `.folio-hidden` is set, bubbles are suppressed in JS (no ghost `aria-live` announcements), the inactivity/sleep clock doesn't run, fidgets skip, and CSS pauses all SVG animations + flips `visibility: hidden` after the fade (drops the scene from the a11y tree and stops paint work).
